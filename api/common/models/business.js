...
var redisInvoicePayed = redis.createClient(config.REDIS.PORT, config.REDIS.HOST)
...
module.exports = function (Business) {
  ...
  Business.observe('before save', function (ctx, next) {
    ...
    function updateModel (business) {
      if (business.mobile) {
        business.mobile = utility.removeAllSpace(utility.verifyAndTrimMobile(business.mobile))
        if (!business.mobile) {
          var error = new Error()
      ...
      if (business.password) {
        business.passwordText = utility.encrypt(business.password, config.ENCRYPTION_KEY)
      }
    }
  ...
  ctx.instance.services = {
    ...
    duration:         1
      ...
      ctx.instance.themeConfig[config.DEFAULT_THEME_ID].style = hotspotTemplates[config.DEFAULT_THEME_ID].styles[0].id
      ctx.instance.themeConfig[config.DEFAULT_THEME_ID].formConfig = hotspotTemplates[config.DEFAULT_THEME_ID].formConfig
      var SystemConfig = app.models.SystemConfig
      SystemConfig.getConfig().then(function (systemConfig) {
      ...
      InternetPlan.destroyAll({businessId: businessId}, function (error, res) {
        ...
        Nas.destroyAll({businessId: businessId}, function (error, res) {
          ...
          Member.destroyAll({businessId: businessId}, function (error, res) {
            ...
            Invoice.destroyAll({businessId: businessId}, function (error, res) {
              if (error) {
                log.error(error)
                return next(error)
              }
  ...
  Business.observe('after save', function (ctx, next) {
    ...
    if (ctx.isNewInstance) {
      ...
      Role.findOne({where: {name: config.ROLES.NETWORKADMIN}}, function (error, role) {
        if (error) {
          log.error('failed to load ' + config.ROLES.NETWORKADMIN + ' for role assignment', error)
          return next()
        }
        if (!role) {
        ...
        role.principals.create(roleMapping, function (error, result) {
          ...
          //Add trial sms test;
          Business.assignDefaultPlanToBusiness(businessId).then(function () {
  ...
  Business.registerNewLicense = function (mobile, fullname, title) {
    return Q.Promise(function (resolve, reject) {
      ...
      needle.request('post', config.CONFIG_SERVER_NEW_LICENSE, {
        'mobile': mobile,
        'title': title,
        'fullName': fullname
      }, {json: true}, function (error, resp, body) {
        if (error) {
          log.error(error)
          return reject(error)
        }
        ...
        utility.writeStringToFileInPath(config.SYSTEM_ID_PATH, body.systemUuid).then(function () {
  ...
  Business.loadConfig = function (bizId, cb) {
    Business.findById(bizId, function (error, business) {
      ...
      business.autoAssignInternetPlan = business.autoAssignInternetPlan || false
      business.defaultInternetPlan = business.defaultInternetPlan || {}
      business.selectedThemeId = business.selectedThemeId || config.DEFAULT_THEME_ID
      if (!business.themeConfig) {
        business.themeConfig = {}
        business.themeConfig[config.DEFAULT_THEME_ID] = {'formConfig': hotspotTemplates[config.DEFAULT_THEME_ID].formConfig}
      }
      ...
      if (!business.formConfig || !underscore.isArray(business.formConfig)) {
        business.formConfig = hotspotTemplates[config.DEFAULT_THEME_ID].formConfig
      }
  ...
  Business.getPackageById = function (packageId) {
    return Q.Promise(function (resolve, reject) {
      if (!packageId || packageId === 'premium' || packageId === 'silver' || packageId === 'gold' || packageId === 'bronze' || packageId === 'free') {
        packageId = 'economic'
      }
  ...
  Business.assignDefaultPlanToBusiness = function (businessId) {
    ...
    return Q.Promise(function (resolve, reject) {
      SystemConfig.isLocal().then(function (isLocal) {
        if (!isLocal) {
          Business.assignPackageToBusiness(businessId, 'demo').then(function () {
            return resolve()
          }).fail(function (error) {
            return reject(error)
          })
  ...
  Business.getCurrentService = function (business) {
    ...
    return Q.Promise(function (resolve, reject) {
      SystemConfig.isLocal().then(function (isLocal) {
        if (isLocal) {
          SystemConfig.getService().then(function (currentService) {
          ...
          if (currentService.allowedOnlineUsers && typeof currentService.allowedOnlineUsers === 'string') {
            currentService.allowedOnlineUsers = Number(currentService.allowedOnlineUsers)
          }
          currentService.allowedOnlineUsers = currentService.allowedOnlineUsers || config.DEFAULT_ALLOWED_ONLINE_USERS
          return resolve(currentService)
        }
      })
    })
  ...
  Business.getModules = function (business) {
    ...
    return Q.Promise(function (resolve, reject) {
      SystemConfig.isLocal().then(function (isLocal) {
        if (isLocal) {
          SystemConfig.getModules().then(function (modules) {
  ...
  Business.buyPackage = function (packageId, discountCoupon, ctx) {
    ...
    return Q.Promise(function (resolve, reject) {
      SystemConfig.isLocal().then(function (isLocal) {
        if (isLocal) {
          utility.getSystemUuid(config.SYSTEM_ID_PATH).then(function (systemUuid) {
            auth.loginToLicenseServer(config.CONFIG_SERVER_LOGIN).then(function (authResult) {
              var token = authResult.token
              var providerId = authResult.userId
              log.debug('REturn to ', config.BUY_LOCAL_PACKAGE_RETURN())
              needle.post(config.CONFIG_SERVER_BUY_PACKAGE.replace('{token}', token), {
          ...
          Business.getPackageById(packageId).then(function (selectedPackage) {
            if (!selectedPackage) {
              return reject('package not found')
            }
            var price = selectedPackage.price - (selectedPackage.price * selectedPackage.discount)
            ...
            function createInvoiceAndPay (price) {
              if (price === 0) {
                Business.assignPackageToBusiness(businessId, packageId).then(function () {
                  var returnUrl = config.BUSINESS_PAYMENT_RESULT_URL()
                  return resolve({url: returnUrl.replace('{0}', 'true').replace('{1}', '&desc=success')})
                }).fail(function (error) {
                  log.error('failed to assign zero price pkg to business', error)
                  return reject(error)
                })
              } else {
                ...
                Invoice.create({
                  ...
                  var returnUrl = config.BUSINESS_PAYMENT_RETURN_URL().replace('{0}', 'invoiceId').replace('{1}', invoiceId)
                  log.debug('config.BUSINESS_PAYMENT_RETURN_URL (): ', config.BUSINESS_PAYMENT_RETURN_URL())
                  log.debug('returnUrl: ', returnUrl)
                  log.debug('EXTRACTED_EXTERNAL_API_ADDRESS: ', process.env.EXTRACTED_EXTERNAL_API_ADDRESS)
                  Payment.openPaymentGateway(config.PAYMENT_API_KEY, price, config.PAYMENT_GATEWAY_DEFAULT_DESC, config.PAYMENT_SUPPORT_EMAIL, config.PAYMENT_SUPPORT_MOBILE, returnUrl).then(function (response) {
  ...
  Business.assignPackageToBusiness = function (businessId, packageId, options) {
    return Q.Promise(function (resolve, reject) {
      options = options || {}
      Business.getPackageById(packageId).then(function (selectedPkg) {
        if (!selectedPkg) {
        ...
        Business.findById(businessId, function (error, business) {
          ...
          var durationInDays = options.durationInDays || selectedPkg.durationInDays
          durationInDays = Number(durationInDays)
          var selectedService = selectedPkg.service
          if (selectedService) {
            var serviceSubscriptionDate = options.subscriptionDate || new Date().getTime()
            log.debug('service duration:', duration)
            var expiresAt
            if (duration) {
              expiresAt = (new Date(serviceSubscriptionDate).add({months: duration})).getTime()
            } else if (durationInDays) {
              expiresAt = (new Date(serviceSubscriptionDate).add({days: durationInDays})).getTime()
            }
            log.debug('service expires at:', expiresAt)
            update.services = {
              allowedOnlineUsers: options.allowedOnlineUsers || selectedService.allowedOnlineUsers,
          ...
          if (selectedModules) {
            ...
            underscore.each(selectedModules, function (module, moduleId) {
              var modSubscriptionDate = options.subscriptionDate || new Date().getTime()
              var modExpiresAt
              log.debug('module duration ', duration)
              if (duration) {
                modExpiresAt = (new Date(modSubscriptionDate).add({months: duration})).getTime()
              } else if (durationInDays) {
                modExpiresAt = (new Date(modSubscriptionDate).add({days: durationInDays})).getTime()
              }
        ...
        if (isLocal) {
          utility.getSystemUuid(config.SYSTEM_ID_PATH).then(function (systemUuid) {
            auth.loginToLicenseServer(config.CONFIG_SERVER_LOGIN).then(function (authResult) {
              var token = authResult.token
              var providerId = authResult.userId
              needle.post(config.CONFIG_SERVER_CHARGE_SMS.replace('{token}', token), {
                systemUuid: systemUuid,
                providerId: providerId,
          ...
          Invoice.create({
            ...
            var invoiceId = invoice.id
            var returnUrl = config.CHARGE_PAYMENT_RETURN_URL().replace('{0}', 'invoiceId').replace('{1}', invoiceId)
            log.debug(returnUrl)
            Payment.openPaymentGateway(config.PAYMENT_API_KEY, price, config.PAYMENT_GATEWAY_DEFAULT_DESC, config.PAYMENT_SUPPORT_EMAIL, config.PAYMENT_SUPPORT_MOBILE, returnUrl)
              .then(function (response) {
                var url = response.url
                var paymentId = response.paymentId
                invoice.updateAttributes({'paymentId': paymentId}).then(function () {
                  return resolve({url: url})
                }, function (error) {
                  log.error('invoice update error:', error)
                  log.error(error)
  ...
  Business.observe('loaded', function (ctx, next) {
    ...
    if (ctx.data) {
      Business.getCurrentService(ctx.data).then(function (currentService) {
        ctx.data.services = currentService
        Business.getModules(ctx.data).then(function (modules) {
          ...
          } else if (ctx.data.selectedThemeId === config.PREVIOUS_DEFAULT_THEME_ID) {
            themeId = config.DEFAULT_THEME_ID
            themeConfig[themeId] = extend({}, returnThemeConfig(themeId, oldThemeConfig, serviceId))
          } else if (ctx.data.selectedThemeId === config.PREVIOUS_HOTEL_THEME_ID) {
        ...
        if (oldThemeConfig.formConfig) {
          for (var i = 0; i < newThemeConfig.formConfig.length; i++) {
            for (var j = 0; j < oldThemeConfig.formConfig.length; j++) {
              if (newThemeConfig.formConfig[i].label === oldThemeConfig.formConfig.label) {
                newThemeConfig.formConfig[i].active = oldThemeConfig.formConfig[j].active
                newThemeConfig.formConfig[i].required = oldThemeConfig.formConfig[j].required
              }
            }
          }
        }
  ...
  Business.verifyBuyPackage = function (invoiceId) {
    return Q.Promise(function (resolve, reject) {
      ...
      Invoice.findById(invoiceId, function (error, invoice) {
        ...
        Payment.verifyPayment(config.PAYMENT_API_KEY, paymentId, price).then(function (result) {
          log.debug(result)
          if (!result.payed) {
            return resolve({
              code: 302,
              ...
              Business.assignPackageToBusiness(businessId, packageId).then(function () {
                Business.getPackageById(packageId).then(function (selectedPackage) {
                  ...
                  if (business.resellerId) {
                    Reseller.addResellerCommission(business.resellerId, businessId, invoice.price).then(function () {
                  ...
                  return resolve({
                    code: 302,
                    returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Error in update business with packageId')
                  })
                })
              }).fail(function (error) {
                log.error(error)
        ...
        if (isLocal) {
          utility.getSystemUuid(config.SYSTEM_ID_PATH).then(function (systemUuid) {
            auth.loginToLicenseServer(config.CONFIG_SERVER_LOGIN).then(function (authResult) {
              ...
              needle.post(config.CONFIG_SERVER_LOCAL_CHARGE.replace('{token}', token), {
                systemUuid: systemUuid,
              }, {json: true}, function (error, response, body) {
  ...
  Business.getTrafficUsage = function (startDate, endDate, offset, monthDays, ctx, cb) {
    var businessId = ctx.currentUserId
    ...
    Business.findById(businessId, function (error, business) {
      ...
      return Usage.getBusinessUsageReport(fromDate, toDate, businessId, offset, intervalMili, monthDays).then(function (result) {
  ...
  Business.getNetflowReport = function (fromDate, toDate, username, sourceIp, destinationIp, skip, limit, ctx, cb) {
    ...
    options.skip = skip
    options.limit = limit
    aggregate.getNetflowLog(options).then(function (netflowLog) {
      var result = {}
      var data = []
  ...
  Business.getSyslogReport = function (fromDate, toDate, username, skip, limit, ctx, cb) {
    var businessId = ctx.currentUserId
    ...
    options.skip = skip
    options.limit = limit
    aggregate.getSyslog(options).then(function (syslog) {
      var result = {}
      var data = []
      var logs = syslog.log
      if (logs.length > 0) {
...
if ( totalDurationInMonths <= 0 || !totalDurationInMonths ) {
  ...
  return reject ( 'business service has no duration' );
      ...
      if (now.between(fromDate, endDate)) {
        log.warn('This business has valid subscription ', business.title, businessId)
        return resolve(true)
      } else {
  ...
  Business.getPackages = function () {
    return Q.Promise(function (resolve, reject) {
      ...
      SystemConfig.isLocal().then(function (isLocal) {
        if (isLocal) {
          needle.post(config.CONFIG_SERVER_LOCAL_MODULES, {}, {json: true}, function (error, response, body) {
            if (error) {
              log.error(error)
              return reject(error)
            }
  ...
  Business.loadServices = function (cb) {
    Business.getPackages().then(function (pkgs) {
    ...
    return Q.Promise(function (resolve, reject) {
      ...
      Business.findById(businessId, function (error, business) {
        ...
        result.then(function (members) {
          InternetPlan.find({where: {businessId: businessId}}).then(function (internetPlans) {
            MemberGroup.find({where: {businessId: businessId}}).then(function (memberGroups) {
              return resolve({
  ...
  Business.restoreBackupFromApi = function (url, ctx) {
    return Q.Promise(function (resolve, reject) {
      ...
      needle.get(url, {json: true}, function (error, response, body) {
        ...
        Business.restoreBackup(body, ctx).then(function (result) {
  ...
  Business.restoreBackup = function (backup, ctx) {
    ...
    return Q.Promise(function (resolve, reject) {
      ...
      backup.memberGroups.forEach(function (memberGroup) {
        createMemberGroupTasks.push(function (memberGroupId) {
          return function (memberGroupDic) {
            ...
            return MemberGroup.create(memberGroup).then(function (createdMemberGroup) {
      ...
      memberGroupResult.then(function (memberGroupCreateResult) {
        ...
        internetPlanResult.then(function (createInternetPlanResult) {

          var createMembersTasks = []
          backup.members.forEach(function (member) {
            createMembersTasks.push(function () {
              return function () {
                var options = {
                  businessId: businessId,
                  ...
                  internetPlanHistory: [],
                  internetPlanId: createInternetPlanResult[member.internetPlanId],
                  groupIdentity: member.groupIdentity,
                  groupIdentityId: memberGroupCreateResult[member.groupIdentityId],
          ...
          memberResult.then(function () {
            Business.findById(businessId).then(function (business) {
              business.updateAttributes({groupMemberCounter: backup.groupMemberCounter || config.BUSINESS_GROUP_MEMBER_COUNTER_START}, function (error) {
                if (error) {
                  return reject(error)
                }
  ...
  Business.destroyMembersById = function (memberIds, ctx) {
    return Q.promise(function (resolve, reject) {
      ...
      for (var i = 0; i < memberIds.length; i++) {
        (function () {
          ...
          tasks.push(Q.Promise(function (resolve, reject) {
            Member.destroyById(memberId, {businessId: businessId}, function (error, res) {
              if (error) {
                log.error(error)
                return reject(error)
              }
      ...
      Q.all(tasks).then(function (resultArray) {
  ...
  Business.dropBoxAuthorization = function (ctx, cb) {
    ...
    Business.findById(businessId, function (error, business) {
      ...
      var dropBoxAuthUrl = DROPBOX_AUTH_URL.replace('{0}', appKey).replace('{1}', redirectURI).replace('{2}', CSRFToken)
      return cb(null, {
        code: 302,
  ...
  Business.dropboxSaveToken = function (options) {
    return Q.Promise(function (resolve, reject) {
      ...
      if (!options) {
        return resolve({
          code: 302,
          returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=No response')
        })
      }
      if (options.error && options.error === 'access_denied') {
        return resolve({
          code: 302,
          returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Access denied')
        })
      }
      if (options.error) {
        return resolve({
          code: 302,
          returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Error in connecting to dropbox')
        })
      }
      var Business = app.models.Business
      ...
      Business.findById(businessId, function (error, business) {
        ...
        request({
          ...
          if (error) {
            ...
            return resolve({
              code: 302,
              returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=' + error)
            })
          }
          try {
            ...
            if (access_token && account_id) {
              business.updateAttributes({
                ...
                if (error) {
                  ...
                  return resolve({
                    code: 302,
                    returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Error in update business with new Dropbox token')
                  })
                }
                log.debug('@Dropbox token saved successfully for business: ', businessId)
                return resolve({
                  code: 302,
                  returnUrl: returnUrl.replace('{0}', 'true').replace('{1}', '&desc=success')
                })
              ...
              return resolve({
                code: 302,
                returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=invalid access token or account id')
              })
            }
          } catch (error) {
            log.error(error)
  ...
  Business.createLocalInvoice = function (businessId, mobile, moduleId, duration) {
    return Q.Promise(function (resolve, reject) {
      var Invoice = app.models.Invoice
      ...
      Invoice.create({
        ...
        var invoiceId = invoice.id
        var returnUrl = config.LOCAL_PAYMENT_RETURN_URL().replace('{0}', 'invoiceId').replace('{1}', invoiceId)
        Payment.openPaymentGateway(config.PAYMENT_API_KEY, price, config.PAYMENT_GATEWAY_DEFAULT_DESC, config.PAYMENT_SUPPORT_EMAIL, config.PAYMENT_SUPPORT_MOBILE, returnUrl).then(function (response) {
          var url = response.url
          var paymentId = response.paymentId
          invoice.updateAttributes({
          ...
          utility.sendMessage('failed to open local payment gateway not found', {error: error})
          log.error('failed to open payment gateway')
          log.error(error)
          return reject(error)
        })
