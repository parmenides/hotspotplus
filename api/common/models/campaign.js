				...
				Business.findById ( businessId, function ( error, business ) {
					...
					Business.hasValidSubscription ( business ).then ( function () {
						Member.count ( {
							...
							var totalCost = Member.calculateBulkSmsCost ( receptorsLength, tempMessage );
												...
												(function () {
													...
													if ( sendCoupon == true ) {
														var coupon = {
															...
															code:         couponPrefix + Math.floor ( (Math.random () * 10000) + 1000 ),
															value:        discount,
															ownerId:      businessId,
														...
														Coupon.create ( coupon, function ( error, result ) {
															if ( error ) {
																log.error ( error );
																return reject ( error );
															}
															var couponMessage = config.PERSIAN_COUPON_MESSAGE.replace ( '{0}', result.code ).replace ( '{1}', result.value.amount );
															switch ( result.value.unit ) {
																case config.PERCENT_UNIT:
																	couponMessage = couponMessage.replace ( '{2}', config.PERSIAN_PERCENT_UNIT );
																	break;
																case config.TOMAN_UNIT:
																	couponMessage = couponMessage.replace ( '{2}', config.PERSIAN_TOMAN_UNIT );
																	break;
																default:
																	break;
															}
															...
															log.debug ( 'bulk message added to Queue', businessId, mobile, message );
															campaign.updateAttributes ( { log: campaignLog }, function ( error, res ) {
																if ( error ) {
																	log.error ( '@sendBulkMessages', error );
																	return reject ( error );
																}
																log.debug ( 'campaign updated', campaignLog );
																return resolve ( 'bulk message added to Queue' );
															} );
														} );
														...
														logData.discount = discount;
														campaignLog.push ( logData );
														log.debug ( 'bulk message added to Queue', businessId, mobile, messageBody );
														campaign.updateAttributes ( { log: campaignLog }, function ( error, res ) {
															if ( error ) {
																log.error ( '@sendBulkMessages', error );
																return reject ( error );
						...
						log.error ( '@sendBulkMessages', error );
						log.debug ( '@sendBulkMessages', "business does not have valid subscription" );
