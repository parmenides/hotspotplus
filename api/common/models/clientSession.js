...
module.exports = function ( ClientSession ) {
	...
	ClientSession.getOnlineUsers = function ( startDate, businessId, skip, limit, cb ) {
		if ( !businessId ) {
		...
		ClientSession.find ( {
			...
			function ( error, sessionList ) {
				...
				if ( sessionList.length == 0 ) {
					...
					if ( sessionList.length > 0 ) {
						Usage.getSessionUsage ( sessionList ).then ( function ( result ) {
	...
	ClientSession.getOnlineSessionCount = function ( businessId, cb ) {
		...
		ClientSession.find ( { where: { businessId: businessId } },
	...
	ClientSession.killOnlineSession = function ( session, ctx, cb ) {
		...
		ClientSession.findOne ( {
			where: { and: [ { memberId: memberId }, { businessId: businessId }, { id: sessionId } ] }
