...
module.exports = function ( FootTraffic ) {
	/* return array of new , return & total count of visitors base on interval date from aggregates.js
	 ...
	 businessId: String
	 */
	FootTraffic.getVisitorsChart = function ( startDate, endDate, businessId, offset, monthDays, cb ) {
		...
		Business.findById( businessId, function ( error, business ) {
			...
			var minVisitTime = (business.minVisitTime || config.MIN_VISIT_TIME_DEFAULT) * config.AGGREGATE.MINUTE_MILLISECONDS
			var maxVisitTime = (business.maxVisitTime || config.MAX_VISIT_TIME_DEFAULT) * config.AGGREGATE.MINUTE_MILLISECONDS

			aggregate.visitorsInterval( fromDate, toDate, intervalMili, minVisitTime, maxVisitTime, offset, businessId ).then( function ( returnDoc ) {
				...
				if ( monthDays.length == 0 ) {
					...
					for ( var reVisitors in returnDoc ) {
						...
						response.reVisitors[ reVisitors ] = returnDoc[ reVisitors ].reVisitors
						response.newVisitors[ reVisitors ] = returnDoc[ reVisitors ].newVisitors
					}
				} else {
				...
				log.debug( 'process of getting visitors info completed successfully' + JSON.stringify( response ) )
	...
	/* return total count of new visitors and growth rate from aggregates.js
	 ...
	 businessId: String
	 */
	FootTraffic.getVisitorsPercent = function ( startDate, endDate, businessId, cb ) {
		var fromDate = Number.parseInt( startDate )
		var toDate = Number.parseInt( endDate )
		var Business = app.models.Business
		// calculate mirror date
		var fromDateMirror = 2 * Number.parseInt( startDate ) - Number.parseInt( endDate )
		...
		Business.findById( businessId, function ( error, business ) {
			...
			aggregate.newVisitors( fromDate, toDate, minVisitTime, maxVisitTime, businessId ).then( function ( visitors ) {
				aggregate.newVisitors( fromDateMirror, toDateMirror, minVisitTime, maxVisitTime, businessId ).then( function ( mirrorVisitors ) {
					var response = {}
					var percent = 0
					...
					log.debug( 'process of getting visitors growth Rate completed successfully' + JSON.stringify( response ) )
					return cb ( null, response )
				} ).fail( function ( error ) {
					log.error( error )
					return cb ( error )
	...
	FootTraffic.remoteMethod( 'getVisitorsPercent', {
		'description': 'Find total count of new visitors and growth rate from data source.',
		accepts:       [
			{
				arg:           'startDate',
				type:          'string',
	...
	FootTraffic.getDailyVisitsChart = function ( startDate, endDate, timeInterval, offset, businessId, cb ) {
		var fromDate = Number.parseInt( startDate )
		var toDate = Number.parseInt( endDate )
		var numOfDays = Number.parseInt( (endDate - startDate) / config.AGGREGATE.DAY_MILLISECONDS )
		var Business = app.models.Business
		...
		Business.findById( businessId, function ( error, business ) {
			...
			var minWalkByTime = (business.minWalkByTime || config.MIN_WALKBY_TIME_DEFAULT) * config.AGGREGATE.MINUTE_MILLISECONDS
			var maxWalkByTime = (business.maxWalkByTime || config.MAX_WALKBY_TIME_DEFAULT) * config.AGGREGATE.MINUTE_MILLISECONDS
			var minVisitTime = (business.minVisitTime || config.MIN_VISIT_TIME_DEFAULT) * config.AGGREGATE.MINUTE_MILLISECONDS
			var maxVisitTime = (business.maxVisitTime || config.MAX_VISIT_TIME_DEFAULT) * config.AGGREGATE.MINUTE_MILLISECONDS

			offset = -offset * config.AGGREGATE.HOUR_MILLISECONDS

			aggregate.visitsInterval( fromDate, toDate, timeInterval, minWalkByTime, maxWalkByTime, offset, businessId ).then( function ( walkBysDoc ) {
				aggregate.visitsInterval( fromDate, toDate, timeInterval, minVisitTime, maxVisitTime, offset, businessId ).then( function ( visitsDoc ) {
					var response = { walkBys: [], visits: [] }
					// log.debug("walkBysDoc:", walkBysDoc);
					// log.debug("visitsDoc:", visitsDoc);

					for ( var i = 0; i < walkBysDoc.length; i++ ) {
						var walkBysHour = Number.parseInt( new Date ( walkBysDoc[ i ].key ).getHours() )
						if ( i < 24 ) {
							response.walkBys[ walkBysHour ] = walkBysDoc[ i ].doc_count
						} else {
							response.walkBys[ walkBysHour ] = response.walkBys[ walkBysHour ] + walkBysDoc[ i ].doc_count
						}
					...
					for ( var walkBy in response.walkBys ) {
						response.walkBys[ walkBy ] = Number.parseInt( response.walkBys[ walkBy ] / numOfDays )
					}
					// log.debug("walkBys after average:", response.walkBys);

					for ( var j = 0; j < visitsDoc.length; j++ ) {
						var visitsHour = Number.parseInt( new Date ( visitsDoc[ j ].key ).getHours() )
						if ( j < 24 ) {
							response.visits[ visitsHour ] = visitsDoc[ j ].doc_count
						} else {
							response.visits[ visitsHour ] = response.visits[ visitsHour ] + visitsDoc[ j ].doc_count
						}
					...
					for ( var visit in response.visits ) {
						response.visits[ visit ] = Number.parseInt( response.visits[ visit ] / numOfDays )
					}
					// log.debug("visits after average:", response.visits);

					log.debug( 'process of getting average visits and walk-bys info completed successfully' + JSON.stringify( response ) )
					return cb ( null, response )
				} ).fail( function ( error ) {
					log.error( error )
					return cb ( error )
	...
	FootTraffic.getVisitsPercent = function ( startDate, endDate, businessId, cb ) {
		...
		Business.findById( businessId, function ( error, business ) {
			...
			aggregate.totalVisits( fromDate, toDate, minVisitTime, maxVisitTime, businessId ).then( function ( visits ) {
				aggregate.totalVisits( fromDateMirror, toDateMirror, minVisitTime, maxVisitTime, businessId ).then( function ( mirrorVisits ) {
					var response = {}
					var percent = 0
					// calculate growth percent
					...
					log.debug( 'process of getting visits growth Rate completed successfully' + JSON.stringify( response ) )
					return cb ( null, response )
				} ).fail( function ( error ) {
					log.error( error )
					return cb ( error )
