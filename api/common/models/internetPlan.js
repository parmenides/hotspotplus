			...
			InternetPlan.findById ( planId, function ( error, plan ) {
				...
				Member.findById ( memberId, function ( error, member ) {
					...
					Business.findById ( member.businessId, function ( error, business ) {
						...
						if ( isFree && (business.defaultInternetPlan && business.defaultInternetPlan.id) && plan.id == business.defaultInternetPlan.id ) {
							activateDefaultPlanCount = 1;
							if ( member.activateDefaultPlanCount ) {
								activateDefaultPlanCount = member.activateDefaultPlanCount || 0
								var period = (business.defaultInternetPlan.period * config.AGGREGATE.HOUR_MILLISECONDS)
								if ( member.subscriptionDate ) {
									subscriptionDate = new Date ( member.subscriptionDate ).getTime ()
								}
								var expireTime = subscriptionDate + period
								if ( expireTime > dateNow && member.activateDefaultPlanCount < business.defaultInternetPlan.count ) {
									activateDefaultPlanCount = member.activateDefaultPlanCount + 1
...
*/
						member.updateAttributes ( {
							...
							InternetPlan.updateInternetPlanHistory ( memberId, planId ).then ( function () {
	...
	InternetPlan.assignFreePlanToMember = function ( memberId, planId, clbk ) {
		...
		InternetPlan.assignPlanToMember ( memberId, planId, true ).then ( function () {
