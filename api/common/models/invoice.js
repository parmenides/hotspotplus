...
module.exports = function ( Invoice ) {
	...
	Invoice.issueExternalInvoiceAndOpenPayment = function ( price, uniqueId, invoiceType, returnUrl, serviceInfo, discountCoupon ) {
		return Q.Promise ( function ( resolve, reject ) {
			discountCoupon = discountCoupon || {};
			Invoice.verifyAndUseCoupon ( price, discountCoupon.code ).then ( function ( discountedPrice ) {
				...
				Invoice.create ( {
					...
					var returnUrl = config.EXTERNAL_PAYMENT_RETURN_URL ().replace ( '{0}', 'invoiceId' ).replace ( '{1}', invoiceId );
					Payment.openPaymentGateway ( config.PAYMENT_API_KEY, discountedPrice, config.PAYMENT_GATEWAY_DEFAULT_DESC, config.PAYMENT_SUPPORT_EMAIL, config.PAYMENT_SUPPORT_MOBILE, returnUrl ).then ( function ( response ) {
				...
				Payment.verifyPayment ( config.PAYMENT_API_KEY, paymentId, price ).then ( function ( result ) {
					...
					if ( result.payed ) {
						...
						invoice.updateAttributes ( {
							...
							return resolve ( {
								code:      302,
								returnUrl: invoice.returnUrl.replace ( '{invoiceId}', invoiceId ).replace ( "{status}", "success" )
							} );
						} );
					} else {
						return resolve ( {
							code:      302,
							returnUrl: invoice.returnUrl.replace ( '{invoiceId}', invoiceId ).replace ( "{status}", "failed" )
						} );
					}
				} ).fail ( function ( error ) {
					log.error ( error );
