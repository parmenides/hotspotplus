...
module.exports = function (Member) {
    ...
    Member.signIn = function (businessId, username, password, routerType, nasId, pinCode, mac, cb) {
        ...
        Member.findOne({
            ...
            function (error, member) {
                ...
                Business.isMoreSessionAllowed(businessId).then(function (isAllowedResult) {
                    ...
                    if (!routerType || routerType === "" || routerType === 'undefined') {
                        routerType = 'mikrotik';
                    }
                    routerType = routerType.toLowerCase();
                    msg[AVP['nasId']] = {type: 'string', value: [nasId]};
                    msg[AVP[routerType]['username']] = {type: 'string', value: [username]};
                    //supply mac in order for lock_by_mac functionality
                    msg[AVP[routerType]['mac']] = {type: 'string', value: [mac]};
                    RadiusAdaptor.RadiusMessage(msg).then(function (AccessRequest) {
                        Member.postAuth(AccessRequest, true).then(function (RadiusResponse) {
                            ...
                            Business.findById(bizId, function (error, business) {
                                ...
                                Business.hasValidSubscription(business).then(function () {
                                    var selectedThemeId = business.selectedThemeId;
                                    if (business.themeConfig[selectedThemeId].showPinCode === true) {
                                        var pinCodeValid = (business.hotSpotPinCode === pinCode);
                                    } else {
                                        pinCodeValid = true;
                                    }
                                    ...
                                    log.warn(e);
                                    log.debug("This business does not have valid subscription, service blocked");
                                    var error = new Error();
                                    error.message = hotspotMessages.businessServiceExpired;
                                    error.status = 401;
                                    return cb(error);
                                })
                            })
    ...
    /* return array of new & old count of members base on interval date from aggregates.js
	 ...
	 businessId: String
	 */
    Member.getMembersChart = function (startDate, endDate, businessId, offset, monthDays, cb) {
        ...
        aggregate.newMemberInterval(fromDate, toDate, offset, intervalMilli, businessId).then(function (newMembers) {
            ...
            log.debug('process of getting members info completed successfully' + JSON.stringify(response))
    ...
    Member.observe('after save', function (ctx, next) {
        ...
        if (ctx.isNewInstance) {
            ...
            Role.findOne({where: {name: config.ROLES.HOTSPOTMEMBER}}, function (error, role) {
                if (error) {
                    log.error('failed to load ' + config.ROLES.HOTSPOTMEMBER + ' for role assignment', error)
                    return next(error)
                }
                if (!role) {
    ...
    Member.observe('before save', function (ctx, next) {
        ...
        function doTheThings(member, clbk) {
            ...
            if (member.username && member.businessId) {
                ...
                member.username = Member.createMemberUsername(member.businessId, member.username);
            }
            return clbk();
        }
    });
    ...
    Member.sendPassword = function (businessId, memberId, nasId, host) {
        ...
        return Q.Promise(function (resolve, reject) {
            Member.findOne({
                ...
                var plainPassword = utility.decrypt(member.passwordText, config.ENCRYPTION_KEY)
                var plainUsername = member.username.split('@')[0]

                log.warn(nasId && host)
                if (nasId && host) {
                    Member.createShortSigninUrl(businessId, nasId, host, plainUsername, plainPassword, memberId, function (error, shortUrl) {
                        if (error) {
    ...
    Member.recoverHotspotUser = function (userMobile, host, nasId, businessId) {
        ...
        return Q.Promise(function (resolve, reject) {
            Member.findOne({
                ...
                if (!member || member.mobile !== utility.verifyAndTrimMobile(userMobile)) {
                ...
                return Member.sendPassword(member.businessId, member.id, nasId, host).then(function (response) {
    ...
    Member.loadMemberPassword = function (memberId, businessId, ctx, cb) {
        ...
        Member.findOne({
            ...
            member.passwordText = utility.decrypt(member.passwordText, config.ENCRYPTION_KEY)
            return cb(null, member)
        })
    };
        ...
        return Q.Promise(function (resolve, reject) {
            ...
            Business.findById(businessId, function (error, business) {
                ...
                Business.hasValidSubscription(business).then(function () {
                    ...
                    Member.getMemberByUserName(businessId, username).then(function (member) {
                        log.debug('Loaded User ', member);
                        if (!member) {
                            RadiusResponse.addReplyMessage(Radius_Messages.usernameNotFound)
                            RadiusResponse.setCode(404);
                            return reject(RadiusResponse)
                        }
            ...
            Business.findById(businessId, function (error, business) {
                ...
                Member.getMemberByUserName(businessId, username).then(function (member) {
                    if (!member) {
                        ...
                        if (!internetPlanId) {
                            RadiusResponse.addReplyMessage(Radius_Messages.memberHasNoInternetPlan);
                            RadiusResponse.setCode(401);
                            RadiusResponse.setErrorCode(601);
                            log.error(RadiusResponse);
                            return reject(RadiusResponse);
                        }
                        Business.isMoreSessionAllowed(businessId).then(function (isMoreSessionAllowedResult) {
                            if(isMoreSessionAllowedResult.ok===false){
                                RadiusResponse.addReplyMessage(Radius_Messages.noMoreConnectionAllowed)
                                RadiusResponse.setCode(401);
                            ...
                            InternetPlan.findById(internetPlanId, function (error, internetPlan) {
                                ...
                                if (!internetPlan) {
                                    ...
                                    RadiusResponse.addReplyMessage(Radius_Messages.internetPlanRemoved);
                                    RadiusResponse.setCode(404);
                                    RadiusResponse.setErrorCode(600);
                                    log.error(RadiusResponse);
                                    return reject(RadiusResponse);
                                }
                                internetPlan.bindMemberSigninToMac = internetPlan.bindMemberSigninToMac || false;
                                log.debug("BindMemberSigninToMac:", internetPlan.bindMemberSigninToMac && AccessRequest.getAttribute('mac'));
                                if (internetPlan.bindMemberSigninToMac && AccessRequest.getAttribute('mac')) {
                                    var deviceMac = utility.trimMac(AccessRequest.getAttribute('mac'));
                                    log.debug("DeviceMac :", deviceMac);
                                    log.debug("Mac :", member.mac && member.mac !== deviceMac);
                                    if (member.mac && member.mac !== deviceMac) {
                                        RadiusResponse.addReplyMessage(Radius_Messages.memberLoginBindToMac);
                                        RadiusResponse.setCode(401);
                                ...
                                Member.getMemberCurrentSessions(businessId, member.id).then(function (sessions) {
                                    var numberOfMembersSession = sessions.length;

                                    if (nas.sessionStatus === 'multiSession') {
                                        var allowedSession = business.allowedSession || config.DEFAULT_ALLOWED_MULTI_SESSION;
                                        if (numberOfMembersSession >= allowedSession) {
                                            // send reject, no more session allowed
                                            RadiusResponse.addReplyMessage(Radius_Messages.noMoreSessionAllowed)
                                            RadiusResponse.setCode(401)
                                        ...
                                        if (nas.kickOnSingleSession) {
                                            ...
                                            log.debug('going to disconnect members because of kickOnSingleSession')
                                            ...
                                            if (numberOfMembersSession >= config.DEFAULT_ALLOWED_SINGLE_SESSION) {
                                                log.debug('Reject the request no more session allowed for single session service');
                                                // send reject, no more session allowed
                                                RadiusResponse.addReplyMessage(Radius_Messages.noMoreSessionAllowed)
                                                RadiusResponse.setCode(401);
                                                RadiusResponse.setErrorCode(606);
                                    ...
                                    Member.hasValidSubscriptionDuration(member, internetPlan).then(function (duration) {
                                        Member.getInternetUsage(businessId.toString(), member.id.toString(), duration.from.getTime(), duration.to.getTime()).then(function (usageReport) {
                                            log.warn('usageReport');
                                            log.warn(usageReport);
                                            Member.hasEnoughBulk(internetPlan, usageReport.bulk, member.extraBulk).then(function (remainingBulk) {
                                                RadiusResponse.addReply('accountingUpdateInterval', config.DEFAULT_ACCOUNTING_UPDATE_INTERVAL_SECONDS);
                                                if (remainingBulk > 0) {
                                                    if (nas.sessionStatus === 'multiSession') {
                                                        if (remainingBulk <= config.ACCOUNTING_DC_THRESHHOLD) {
                                                            RadiusResponse.addReply('accountingUpdateInterval', config.FAST_ACCOUNTING_UPDATE_INTERVAL_SECONDS);
                                                        }
                                                    }
                                                }
                                                //To prevent 2^32 limit on acc input octet
                                                if (remainingBulk === 0 || remainingBulk > 3900000000) {
                                                    remainingBulk = 3900000000;
                                                    log.debug("To prevent 2^32 limit on acc input octet, changing bulk to:", remainingBulk);
                                                }
                                                RadiusResponse.addAllowedBulk(remainingBulk);
                                                Member.hasEnoughTime(internetPlan, usageReport.sessionTime).then(function (remainingTimeInSeconds) {
                                                    ...
                                                    if (uploadSpeed > 0 && downloadSpeed > 0) {
                                                        ...
                                                        RadiusResponse.addConnectionSpeed(nas.accessPointType, downloadSpeed, downloadSpeed * burstDownloadFactor, uploadSpeed, uploadSpeed * burstUploadFactor, burstTime);
                                                    }
                                                    RadiusResponse.setCode(200);
                                                    return resolve(RadiusResponse)
                                                }).fail(function () {
                                                    RadiusResponse.addReplyMessage(Radius_Messages.outOfTime);
                                                    RadiusResponse.setCode(401);
                                                ...
                                                RadiusResponse.addReplyMessage(Radius_Messages.outOfBulk)
                                                RadiusResponse.setCode(401)
                                        ...
                                        RadiusResponse.addReplyMessage(Radius_Messages.noActiveSubscription)
                                        RadiusResponse.setCode(401)
                                        RadiusResponse.setErrorCode(605)
                                        log.error(RadiusResponse);
                                        return reject(RadiusResponse)
    ...
    Member.getInternetUsage = function (businessId, memberId, fromDateInMs, toDateInMs) {
        log.debug('@getInternetUsage', " businessId:", businessId, ' MemberId: ', memberId, ' From: ', fromDateInMs, 'to', toDateInMs);
        return Q.Promise(function (resolve, reject) {
            aggregate.getMemberUsage(fromDateInMs, toDateInMs, memberId, businessId).then(function (usage) {
    ...
    Member.hasValidSubscriptionDuration = function (member, internetPlan) {
        return Q.Promise(function (resolve, reject) {
            ...
            var validDuration = getDuration(now, subscriptionDate, numberOfMonthsOrDays, planType, internetPlan.autoResubscribe);
            if (validDuration && validDuration.from && validDuration.to) {
                ...
                if (internetPlan.autoResubscribe === true) {
                    ...
                    Member.autoReSubscribe(member.businessId, member.id).then(function (resubscribedMember) {
                        if (member.subscriptionDate === resubscribedMember.subscriptionDate) {
                            log.error("autoResubscribe failed, Race happened in member subscription update");
                            log.error("subscriptionDate : ", member.subscriptionDate, ":", resubscribedMember.subscriptionDate);
                            return reject("Race happened in member subscription update");
                        }
                        Member.hasValidSubscriptionDuration(resubscribedMember, internetPlan).then(function (result) {
            ...
            function getDuration(now, from, monthsOrDays, planType, isAutoResubscribe) {
                monthsOrDays = Number(monthsOrDays);
                var to = new Date(from.getTime());
                if (planType === 'monthly') {
                    var daysInMonth = Date.getDaysInMonth(from.getFullYear(), from.getMonth())
                ...
                if (now.between(from, to)) {
                    ...
                    monthsOrDays--;
                    if (monthsOrDays > 0 && (planType === 'daily' || planType === 'monthly')) {
                        log.debug("need more checks on next month or day: ", monthsOrDays);
                        return getDuration(now, to, monthsOrDays, planType, isAutoResubscribe)
                    } else {
        ...
        return Q.Promise(function (resolve, reject) {
            try {
                ...
                if (internetPlan.bulk) {
                    allowedBulk = utility.toByte(internetPlan.bulk.value, internetPlan.bulk.type)
                }
                if (extraBulk) {
        ...
        return Q.Promise(function (resolve, reject) {
            ...
            Member.findOne({
                ...
                if (member && member.active) {
                    //Fill in credentials
                    var clearTextPass = utility.decrypt(member.passwordText, config.ENCRYPTION_KEY);
                    return resolve({username: member.username, clearTextPassword: clearTextPass});
                } else {
    ...
    Member.getMemberCurrentSessions = function (businessId, memberId) {
        return Q.promise(function (resolve, reject) {
            ...
            ClientSession.find({where: {and: [{memberId: memberId}, {businessId: businessId}]}}, function (error, sessions) {
                if (error) {
                    return reject()
                }
                sessions = sessions || [];
    ...
    Member.setSession = function (RadiusRequest, uniqueId, businessId, memberId, subscriptionDate, groupIdentity, groupIdentityId, groupIdentityType) {
        log.debug('@setSession for session id: ', uniqueId, " username", RadiusRequest.getAttribute('username'));
        var ClientSession = app.models.ClientSession
        return Q.Promise(function (resolve, reject) {
            ClientSession.findById(uniqueId, function (error, previousSession) {
                ...
                redisClient.set(redisSessionKey, redisSession, 'EX', 2 * config.DEFAULT_MEMBER_SESSION_EXPIRE_IN_SECONDS);
    ...
    Member.accounting = function (RadiusAccountingMessage) {
        ...
        var Business = app.models.Business
        var RadiusResponse = new radiusAdaptor.RadiusResponse(RadiusAccountingMessage)
        return Q.Promise(function (resolve, reject) {
            ...
            Member.getMemberByUserName(businessId, username).then(function (member) {
                if (!member) {
                    log.warn('404 member not found for ', businessId, 'Username ', username)
                    utility.sendMessage('Member not found 404', {
                        accStatusType: RadiusAccountingMessage.getAttribute('acctStatusType'),
                        nasId: nasId,
                        username: username,
                        businessId: businessId
                    });
                ...
                usage.accStatusType = RadiusAccountingMessage.getAttribute('acctStatusType');
                usage.sessionId = RadiusAccountingMessage.getAttribute('sessionId');
                usage.mac = mac;
                usage.download = RadiusAccountingMessage.getAttribute('download') || 0;
                usage.upload = RadiusAccountingMessage.getAttribute('upload') || 0;
                usage.totalUsage = usage.download + usage.upload
                if (!_.isUndefined(RadiusAccountingMessage.getAttribute('sessionTime'))) {
                    usage.sessionTime = RadiusAccountingMessage.getAttribute('sessionTime')
                } else {
                    usage.sessionTime = 0
                }

                Usage.addUsageReport(usage).then(function () {
                    ...
                    if (accStatusType == 8 || accStatusType == 2) {
                        Member.removeSession(RadiusAccountingMessage.getSessionId(), businessId, member.id)
                    } else {
                        Member.setSession(RadiusAccountingMessage, RadiusAccountingMessage.getSessionId(), businessId, member.id, member.subscriptionDate, member.groupIdentity, member.groupIdentityId, member.groupIdentityType);
                    }
    ...
    Member.setMemberMac = function (memberInstance, mac, forceUpdate) {
        return Q.Promise(function (resolve, reject) {
            if (forceUpdate || (!memberInstance.mac && mac)) {
                ...
                log.debug("member already has mac address so no need to update:", memberInstance.mac);
                return reject("member already has mac address so no need to update:", memberInstance.mac);
            }
        });
    };
    ...
    Member.evaluateMemberUsage = function (member, businessId) {
        ...
        return Q.Promise(function (resolve, reject) {
            InternetPlan.findById(internetPlanId, function (error, internetPlan) {
                ...
                Member.hasValidSubscriptionDuration(member, internetPlan).then(function (duration) {
                    Member.getInternetUsage(businessId, memberId, duration.from.getTime(), duration.to.getTime()).then(function (usageReport) {
                        ...
                        Member.hasEnoughBulk(internetPlan, usageReport.bulk, member.extraBulk).then(function (remainingBulk) {
                            ...
                            Member.disconnectAllSessionOfMember(businessId, memberId).then(function () {
                                log.debug("not enough bulk send kill all for ", member.username);
                                return resolve();
                            }).fail(function (error) {
                                log.error(error);
                                return reject(error);
    ...
    Member.disconnectAllSessionOfMember = function (businessId, memberId) {
        return Q.Promise(function (resolve, reject) {
            ...
            Member.getMemberCurrentSessions(businessId, memberId).then(function (sessions) {
    ...
    Member.createShortVerificationUrl = function (businessId, nasId, host, verificationCode, memberId, cb) {
        if (!businessId || !nasId || !host || !verificationCode || !memberId) {
            return cb('invalid params to create verification url');
        }
        var verificationUrl = config.HOTSPOT_VERIFICATION_URL().replace("{businessId}", businessId)
            .replace("{nasId}", nasId).replace("{host}", host).replace('{verificationCode}', verificationCode).replace('{memberId}', memberId);
        var urlKey = utility.createRandomKeyword(6);
        var shortUrl = config.SHORTNER_URL() + urlKey;
        redisClient.set(urlKey, verificationUrl, 'EX', config.SHORT_VERIFICATION_URL_EXPIRES_AT);
        return cb(null, shortUrl);
    };

    Member.createShortSigninUrl = function (businessId, nasId, host, username, password, memberId, cb) {
        var signInUrl = config.HOTSPOT_SIGNIN_URL().replace("{businessId}", businessId)
            .replace("{nasId}", nasId).replace("{host}", host).replace('{username}', username).replace('{password}', password).replace('{memberId}', memberId);
        var urlKey = utility.createRandomKeyword(6);
        var shortUrl = config.SHORTNER_URL() + urlKey;
    ...
    Member.createNewMember = function (options, businessId, nasId, host) {
        ...
        return Q.Promise(function (resolve, reject) {
            ...
            member.username = options.username || utility.verifyAndTrimMobile(member.mobile);
            member.passwordText = options.password || utility.createRandomNumericalPassword();
            ...
            Business.findById(businessId, function (error, business) {
                ...
                Member.create(member, function (error, result) {
                    ...
                    if (options.sendVerificationSms) {
                        Member.sendVerificationMessage(business, nasId, host, member.verificationCode, memberId, member.mobile).then(function (verificationInfo) {
    ...
    Member.sendVerificationMessage = function (business, nasId, host, verificationCode, memberId, mobile) {
        var businessId = business.id;
        return Q.Promise(function (resolve, reject) {
            Member.createShortVerificationUrl(businessId, nasId, host, verificationCode, memberId, function (error, shortUrl) {
                if (error) {
                ...
                var smsVerificationTemplate = config.HOTSPOT_VERIFICATION_MESSAGE_TEMPLATE;
                if (business.callBasedVerification === true) {
                    smsVerificationTemplate = config.HOTSPOT_VERIFICATION_MESSAGE_TEMPLATE_BY_CALL;
                }
                ...
                return resolve({shortUrl: shortUrl, verificationCode: verificationCode});
            });
        })
    };

    Member.createHotSpotMember = function (mobile, firstName, lastName, fullName, gender, birthday, birthDay, birthMonth, birthYear, username, password, email, nationalCode, age, businessId, host, nasId, language, roomNumber, passportNumber, studentGrade, studentId, verificationMethod, cb) {
        ...
        Member.findOne(
            {
                where: {
                    and: [
                        ...
                        {
                            or: [
                                {username: utility.verifyAndTrimMobile(mobile) + "@" + businessId},
                ...
                //Fix findOne bug;
                if (previousMember && previousMember.mobile !== utility.verifyAndTrimMobile(mobile)) {
                    previousMember = null;
                }
                ...
                if (!previousMember) {
                    Member.createNewMember({
                        ...
                        studentId: studentId,
                        age: age
                    }, businessId, nasId, host).then(function (credential) {
                    ...
                    Business.findById(businessId, function (error, business) {
                        ...
                        var verificationCodeMax = (business.MAX_VERIFICATION_COUNT || config.MAX_VERIFICATION_COUNT);
                        if (previousMember.active) {
                            log.error('Member Exist with username: ', previousMember.username);
                            return cb(null, {status: -1})
                        }
                        if (!previousMember.mobile) {
                            log.debug('Member find but needs manual verification ', previousMember.username);
                            return cb(null, {status: 3})
                        }
                        if ((!previousMember.active && previousMember.verificationCount <= verificationCodeMax) || !previousMember.verificationCount) {
                            log.debug('Member find but it is not active, username: ', previousMember.username);
                            var count = previousMember.verificationCount + 1;
                            previousMember.updateAttributes({'verificationCount': count}, function (error, res) {
                                if (error) {
                                    log.error(error);
                                    return cb(error)
                                }
                                Member.sendVerificationMessage(business, nasId, host, previousMember.verificationCode, previousMember.id, previousMember.mobile).then(function () {
                            ...
                            log.debug('Member exist but verification message limit reached: ', previousMember.username);
	 ...
	 */
    Member.verifyHotSpot = function (businessId, memberId, verificationCode, host, nasId, cb) {
            ...
            , function (error, member) {
                ...
                if (member && member.verificationCode == verificationCode) {
                    ...
                    member.updateAttributes({
                        ...
                        var password = utility.decrypt(member.passwordText, config.ENCRYPTION_KEY)
                        if (activatedPreviously !== true) {
                            log.debug('member did not verified previously');
                            Member.sendPassword(businessId, memberId, nasId, host).then(function () {
                                return cb(null, {username: member.username, password: password})
                            }).fail(function (error) {
                                return cb(error)
                            })
                        } else {
    ...
    Member.paySubscription = function (businessId, memberId, internetPlanId, nasId, host, username, password) {
        log.debug('businessId, memberId, internetPlanId, nasId, host, username, password');
        log.debug(businessId, memberId, internetPlanId, nasId, host, username, password);
        return Q.Promise(function (resolve, reject) {
            var Business = app.models.Business;
            var InternetPlan = app.models.InternetPlan;
            Business.findById(businessId, function (error, business) {
                ...
                InternetPlan.findById(internetPlanId, function (error, internetPlan) {
                    ...
                    var desc = internetPlan.name + internetPlan.type + ', ' + internetPlan.speed.value + internetPlan.speed.type + ', ' + internetPlan.bulk.value + internetPlan.bulk.type;
                    ...
                    Invoice.create({
                            ...
                            var returnUrl = config.HOTSPOT_PAYMENT_RETURN_URL().replace('{0}', 'invoiceId')
                                ...
                                .replace('{host}', host);
                            Payment.openPaymentGateway(business.paymentApiKey, price, config.PAYMENT_GATEWAY_INTERNET_PLAN_PAYMENT_DESC, business.email, business.mobile, returnUrl).then(function (response) {
    ...
    Member.verifySubscriptionPayment = function (invoiceId, username, password, host, nasId, memberId, businessId) {
        return Q.Promise(function (resolve, reject) {
            var Invoice = app.models.Invoice
            var InternetPlan = app.models.InternetPlan
            var Business = app.models.Business
            ...
            try {
                ...
                Invoice.findById(invoiceId, function (error, invoice) {
                    ...
                    if (error) {
                        ...
                        return resolve({
                            code: 302,
                            returnUrl: config.HOTSPOT_PAYMENT_WEB_RETURN_URL().replace('{0}', querystring.stringify(params))
                        })
                    }
                    if (!invoice) {
                    ...
                    Business.findById(businessId, function (error, business) {
                        ...
                        Payment.verifyPayment(business.paymentApiKey, paymentId, price).then(function (verificationResult) {
                            ...
                            if (verificationResult.payed) {
                                ...
                                invoice.updateAttributes({
                                    ...
                                    log.debug('Update invoiceId ' + invoiceId + ' with verify payment refId ' + refId)
                                    InternetPlan.assignPlanToMember(invoice.memberId, invoice.planId).then(function () {
                                        log.debug('%%%%%%%%%%%')
                                        log.debug(invoice)
                                ...
                                log.debug('Payment failed ' + config.HOTSPOT_PAYMENT_WEB_RETURN_URL().replace('{0}', false).replace('{1}', 'verify'))
                                params.payStatus = false
                                return resolve({
                                    code: 302,
                                    returnUrl: config.HOTSPOT_PAYMENT_WEB_RETURN_URL().replace('{0}', querystring.stringify(params))
        ...
        return Q.Promise(function (resolve, reject) {
            ...
            Member.findById(memberId, function (error, member) {
                ...
                member.updateAttributes({
                    ...
                    InternetPlan.updateInternetPlanHistory(memberId, member.internetPlanId).then(function () {
                        Member.findById(memberId, function (error, latestMember) {
                            ...
                            log.debug("member resubscribed automatically:", businessId, ":", memberId);
    ...
    Member.getSmsCostForAllMembers = function (businessId, messageText) {
        return Q.Promise(function (resolve, reject) {
            ...
            Member.count({businessId: businessId, mobile: {'gt': 0}}, function (error, receptorsLength) {
                if (error) {
                    log.error('@getSmsCostForAllMembers', error)
                    return reject(error)
                }
                var totalCost = Member.calculateBulkSmsCost(receptorsLength, messageText)
                aggregate.getProfileBalance(businessId).then(function (balance) {
                    var result = {
                        totalCost: totalCost,
            ...
            Business.findById(businessId, function (error, business) {
                ...
                if (business.modules && business.modules.sms) {
                    if (new Date().isBefore(new Date(business.modules.sms.expiresAt))) {
                        smsModule.send(smsData).then(function () {
                            return resolve();
                        }).fail(function (error) {
                            return reject(error);
                        });
        ...
        return Q.Promise(function (resolve, reject) {
            ...
            for (var i = 0; i < members.length; i++) {
                ...
                if (member.subscriptionDate) {
                    ...
                    tasks.push(aggregate.getMemberUsage(fromDate, toDate, id, businessId));
                }
            }
            Q.all(tasks).then(function (trafficResults) {
                var results = {};
    ...
    Member.getMemberBalance = function (businessId, memberId, cb) {
        ...
        Member.findOne({
                ...
                InternetPlan.findOne({
                    ...
                    Member.getInternetUsage(businessId, memberId, fromDateInMs, toDateInMs).then(function (usage) {
                        ...
                        if (plan.timeDuration > 0) {
                            var currentTime = plan.timeDuration - (usage.sessionTime / 60).toFixed(0);
                            if (currentTime < 0) {
                                currentTime = 0;
                            }
    ...
    Member.remoteMethod('getBusinessId', {
        'description': 'Find Business Id from Member User Name And Business Url Prefix',
        accepts: [
            {
                arg: 'businessUrl',
                type: 'string',
    ...
    Member.getDailyUsage = function (businessId, memberId, startDate, cb) {
        ...
        aggregate.getMemberDailyUsage(businessId, memberId, startDate).then(function (memberUsage) {
            var result = {};
            result.date = [];
            result.download = [];
            result.upload = [];
            if (memberUsage.length > 0) {
                ...
                for (var i = 0; i < memberUsage.length; i++) {
                    ...
                    download.push(utility.toMByte(memberUsage[i].sum_download.value).toFixed(0));
                    upload.push(utility.toMByte(memberUsage[i].sum_upload.value).toFixed(0));
                }
                result.date = date;
            ...
            Business.findById(businessId, function (error, business) {
                ...
                InternetPlan.findById(internetPlanId, function (error, internetPlan) {
                    ...
                    Invoice.create({
                            ...
                            var returnUrl = config.MEMBER_PAYMENT_RETURN_URL().replace('{0}', 'invoiceId').replace('{1}', invoice.id);
                            Payment.openPaymentGateway(business.paymentPanelApiKey, price, config.PAYMENT_GATEWAY_INTERNET_PLAN_PAYMENT_DESC, business.email, business.mobile, returnUrl).then(function (response) {
                                var paymentId = response.paymentId;
                                invoice.updateAttributes({
                                    'paymentId': paymentId
                                }, function (error) {
    ...
    Member.verifyPayment = function (invoiceId) {
        return Q.Promise(function (resolve, reject) {
            ...
            if (!invoiceId) {
                return resolve({
                    code: 302,
                    returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=No invoice id')
                })
            }
            Invoice.findById(invoiceId, function (error, invoice) {
                if (error) {
                    ...
                    return resolve({
                        code: 302,
                        returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Error in finding invoice')
                    })
                }
                if (!invoice) {
                    return resolve({
                        code: 302,
                        returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Invalid invoice id')
                    })
                }
                var businessId = invoice.businessId;
                var paymentId = invoice.paymentId;
                ...
                Business.findById(businessId, function (error, business) {
                    if (error) {
                        ...
                        return resolve({
                            code: 302,
                            returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Error in finding business')
                        })
                    }
                    if (!business) {
                        return resolve({
                            code: 302,
                            returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Invalid business id')
                        })
                    }
                    ...
                    if (!business.paymentPanelApiKey) {
                        return resolve({
                            code: 302,
                            returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Invalid payment token id')
                        })
                    }
                    Payment.verifyPayment(business.paymentPanelApiKey, paymentId, price).then(function (verificationResult) {
                        log.warn(verificationResult);
                        ...
                        if (verificationResult.payed) {
                            ...
                            invoice.updateAttributes({
                                ...
                                if (error) {
                                    ...
                                    return resolve({
                                        code: 302,
                                        returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Error in update invoice with payment reference Id')
                                    })
                                }
                                log.debug('Update invoiceId ' + invoiceId + ' with verify payment refId ' + refId)
                                switch (invoiceType) {
                                    case config.BUY_INTERNET_PLAN :
                                        InternetPlan.assignPlanToMember(invoice.memberId, invoice.planId).then(function () {
                                            ...
                                            return resolve({
                                                code: 302,
                                                returnUrl: returnUrl.replace('{0}', 'true').replace('{1}', '&desc=success')
                                            })
                                        }).fail(function (error) {
                                            log.error(error);
                                            return resolve({
                                                code: 302,
                                                returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Error in adding plan to member')
                                            });
                                        })
                                        break;
                                    case config.BUY_EXTRA_BULK :
                                        Member.findById(invoice.memberId, function (error, member) {
                                            log.debug('%%%%%%%%%%%')
                                            log.debug(member)
                                            if (error) {
                                                log.error(error)
                                                return resolve({
                                                    code: 302,
                                                    returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Error in finding member')
                                                })
                                            }
                                            if (!invoice) {
                                                return resolve({
                                                    code: 302,
                                                    returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Invalid member id')
                                                })
                                            }
                                            var description = invoice.description;
                                            var amount = Number.parseInt(description.split(',')[0]);
                                            if (member.extraBulk) {
                                            ...
                                            member.updateAttributes({
                                                ...
                                                if (error) {
                                                    log.error(error);
                                                    log.error('Error in update member : ', invoice.memberId);
                                                    return resolve({
                                                        code: 302,
                                                        returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Error in update invoice with payment reference Id')
                                                    })
                            ...
                            log.debug('Payment failed ' + invoiceId);
                            log.debug('Payment failed ' + returnUrl.replace('{0}', false).replace('{1}', 'verify'));
                            return resolve({
                                code: 302,
                                returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Payment failed')
                            });
                        }
                    }).fail(function (error) {
                        log.error(error)
                        ...
                        return resolve({
                            code: 302,
                            returnUrl: returnUrl.replace('{0}', 'false').replace('{1}', '&error=Error in verifying payment')
                        });
                    });
                });
            });
            ...
            Business.findById(businessId, function (error, business) {
                ...
                InternetPlan.findById(internetPlanId, function (error, internetPlan) {
                    ...
                    Invoice.create({
                            ...
                            var returnUrl = config.MEMBER_PAYMENT_RETURN_URL().replace('{0}', 'invoiceId').replace('{1}', invoice.id);
                            Payment.openPaymentGateway(business.paymentPanelApiKey, totalPrice, config.PAYMENT_GATEWAY_INTERNET_BULK_PAYMENT_DESC, business.email, business.mobile, returnUrl).then(function (response) {
                                var paymentId = response.paymentId;
                                invoice.updateAttributes({
                                    'paymentId': paymentId
                                }, function (error) {
    ...
    Member.checkDefaultPlan = function (businessId, memberId, planId, cb) {
        ...
        Business.findById(businessId, function (error, business) {
            ...
            Member.findById(memberId, function (error, member) {
                ...
                var defaultPlanTime = subscriptionDate + (defaultPlanPeriod * config.AGGREGATE.HOUR_MILLISECONDS);
                if (memberDefaultPlanCount < defaultPlanCount || now > defaultPlanTime) {
                    return cb(null, true);
                } else {
    ...
    Member.importMemberFromCsv = function (csvString, internetPlanId, ctx) {
        ...
        return Q.Promise(function (resolve, reject) {
            ...
            Business.findById(businessId, function (error, business) {
                ...
                csvtojson({noheader: false}).fromString(csvString)
                    ...
                    for (var i in usersOptions) {
                        addMemberTasks.push(function (option, businessId) {
                            return function () {
                    ...
                    addMemberTaskResult.then(function () {
            ...
            Nas.findById(nasId, function (error, nas) {
                ...
                Member.findOne({where: {mac: mac}}, function (error, member) {
                    ...
                    return resolve({
                        ...
                        password: utility.decrypt(member.passwordText, config.ENCRYPTION_KEY)
                    })
                });
    ...
    Member.renderMemberCredentials = function (membersList, helpText, direction) {
        ...
        return Q.Promise(function (resolve, reject) {
            ...
            fs.readFile(config.VOUCHER_TEMPLATE_PATH, {'encoding': 'utf-8'},
                function (error, voucherTemplates) {
                    if (error) {
                        log.error(error);
                        return reject(error);
                    }
                    dust.loadSource(dust.compile(voucherTemplates, 'memberPrintTemplate', false));
                    var data = {members: pagesMembers, help: helpText, direction: direction || "rtl"};
                    log.debug(data);
    ...
    Member.createMembersByGroup = function (reserveCode, tourCode, mobile, passportNumber, nationalCode, count, internetPlanId, duration, helpLanguageCode, ctx, cb) {
        ...
        Business.findById(businessId, function (error, business) {
            ...
            if (!mobile && !reserveCode && !tourCode && !nationalCode && !passportNumber) {
                return cb("invalid parameters");
            }

            var counterCurrentValue = business.groupMemberCounter || config.BUSINESS_GROUP_MEMBER_COUNTER_START;
            ...
            business.updateAttributes({
                ...
                var memberGroupData = {
                    groupIdentity: (mobile || reserveCode || tourCode || nationalCode || passportNumber),
                    mobile: mobile,
                    tourCode: tourCode,
                    groupIdentityType: groupIdentityType,
                ...
                MemberGroup.create(memberGroupData, function (error, memberGroup) {
                    ...
                    for (var i = counterCurrentValue; i < counterNewValue; i++) {
                        tasks.push(function (j) {
                            return function (memberList) {
                                ...
                                return Member.createNewMember(options, businessId).then(function (theMember) {
                    ...
                    result.then(function (result) {
                        ...
                        if (business.groupMemberHelps && helpLanguageCode && business.groupMemberHelps[helpLanguageCode]) {
                            help = business.groupMemberHelps[helpLanguageCode];
                        }
                        Member.renderMemberCredentials(result, help, direction).then(function (renderedHtmlAsString) {
