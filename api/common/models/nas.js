...
module.exports = function (Nas) {
    ...
    Nas.loadThemeConfigById = function (nasId, cb) {
        ...
        Nas.loadRouterInfo(nasId, function (error, nas) {
            ...
            Business.loadConfig(nas.businessId, function (error, businessConfig) {
                ...
                businessConfig.themeConfig.controller = hotspotTemplates[businessConfig.selectedThemeId].controller;
                businessConfig.themeConfig.template = hotspotTemplates[businessConfig.selectedThemeId].template;
    ...
    Nas.getStatus = function (businessId, cb) {
        ...
        var NAS_STATUS_CHECK_IN_MILLISECONDS = config.NAS_STATUS_CHECK_IN_SECONDS * 1000;
        ...
        return Q.Promise(function (resolve, reject) {
            ...
            Business.findById(businessId, function (error, business) {
                ...
                Nas.findById(nasId, function (error, nas) {
                    ...
                    var businessServicesExpiresAt = (new Date()).add({months: 1}).getTime();
                    ...
                    if (business.modules && business.modules.log && business.modules.log.expiresAt) {
                        hasValidLogSubscription = new Date().isBefore(new Date(business.modules.log.expiresAt));
                    }
                    var sessionString = JSON.stringify({
                        ...
                        sharedSecret: business.nasSharedSecret || config.PRIMARY_SHARED_SECRET,
                        nasId: nasId,
                        nasIp: ip,
                        nasPort: port
                    });
                    log.debug("Ip session updated on MongoDB businessId: ", businessId, " nasId: ", nasId, " ip: ", ip);
                    redisClient.set(ip, sessionString, 'EX', config.NAS_SESSION_EXPIRES_AT, function (error, reply) {
                        if (error) {
                            log.error("Setting nas session failed ", error);
                            return reject(error);
                        }
                        log.debug("Set nas session updated on REDIS businessId: ", businessId, " nasId: ", nasId, " ip: ", ip);
                        return resolve();
                    });
