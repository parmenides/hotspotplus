...
module.exports = function ( Reseller ) {
	...
	Reseller.addResellerCommission = function ( resellerId, businessId, price ) {
		return Q.Promise ( function ( resolve, reject ) {
			...
			Reseller.findById ( resellerId, function ( error, reseller ) {
				...
				var commission = price * ( reseller.commissionRate || config.SERVICES.RESELLERS_COMMISSION_RATE);
				log.debug ( {
	...
	Reseller.getBalance = function ( resellerId ) {
		...
		return Q.Promise ( function ( resolve, reject ) {
			aggregate.getProfileBalance ( resellerId ).then ( function ( balance ) {
				log.debug ( balance )
	...
	Reseller.findBusiness = function ( resellerId, businessId, cb ) {
		...
		Business.findOne ( { where: { and: [ { resellerId: resellerId }, { id: businessId } ] } }, function ( error, result ) {
			if ( error ) {
				log.error ( error )
				return cb ( error )
			}
	...
	Reseller.removeBusiness = function ( resellerId, businessId, cb ) {
		...
		Business.findOne ( { where: { and: [ { resellerId: resellerId }, { id: businessId } ] } }, function ( error, Business ) {
			if ( error ) {
				log.error ( error )
				return cb ( error )
			}
	...
	Reseller.assignPackageToReseller = function ( resellerId, allowedOnlineUsers, durationInMonths ) {
		return Q.Promise ( function ( resolve, reject ) {
			if ( !resellerId ) {
				return reject ( "invalid resellerId" );
			}
	...
	Reseller.observe ( 'after save', function ( ctx, next ) {
		...
		if ( ctx.isNewInstance ) {
			...
			Role.findOne ( { where: { name: config.ROLES.RESELLER } }, function ( error, role ) {
				if ( error ) {
	...
	Reseller.observe ( 'before save', function ( ctx, next ) {
		...
		function updateModel ( reseller ) {
			...
			if ( reseller.password ) {
				reseller.passwordText = utility.encrypt ( reseller.password, config.ENCRYPTION_KEY );
			}
		}
		...
		if ( ctx.instance && ctx.isNewInstance ) {
			...
			ctx.instance.allowedOnlineUsers = ctx.instance.allowedOnlineUsers || config.DEFAULT_RESELLER_ONLINE_USERS;
			ctx.instance.durationInMonths = ctx.instance.durationInMonths || config.DEFAULT_RESELLER_DURATION_MONTHS;
			ctx.instance.planType = ctx.instance.planType || config.DEFAULT_RESELLER_PLAN_TYPE;
		}
		next ()
	} );
