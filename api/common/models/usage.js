...
var kafkaClient = new kafka.KafkaClient ( { kafkaHost: process.env.KAFKA_IP + ":" + process.env.KAFKA_PORT } );
...
module.exports = function ( Usage ) {
	...
	Usage.getBusinessUsageReport = function ( startDate, endDate, businessId, offset, intervalMili, monthDays ) {

		return Q.Promise ( function ( resolve, reject ) {
			if ( utility.isMongoDbStorage () ) {
				aggregate.getBusinessDailyUsageReport ( businessId, startDate, endDate ).then ( function ( result ) {
				...
				aggregate.getUniqueSessionCount ( businessId, startDate, endDate ).then ( function ( sessionCount ) {
					if ( sessionCount < config.DEFAULT_AGGREGATION_SIZE ) {
						sessionCount = config.DEFAULT_AGGREGATION_SIZE;
					}
					aggregate.getMemberTrafficUsageReport ( startDate, endDate, businessId, offset, intervalMili, sessionCount ).then ( function ( memberResult ) {
						...
						if ( monthDays.length === 0 ) {
							...
							for ( var i = 0; i < memberResult.length; i++ ) {
								...
								response.download[ i ] = utility.toMByte ( memberResult[ i ].download.value ).toFixed ( 0 )
								response.upload[ i ] = utility.toMByte ( memberResult[ i ].upload.value ).toFixed ( 0 )
							}
						} else {
							...
							for ( var month in monthDays ) {
								...
								response.download[ month ] = utility.toMByte ( downloads ).toFixed ( 2 )
								response.upload[ month ] = utility.toMByte ( uploads ).toFixed ( 2 )
							}
						}
						log.debug ( 'process of getting traffic usage info completed successfully' + JSON.stringify ( response ) )
	...
	Usage.getSessionUsage = function ( sessionList ) {
		return Q.Promise ( function ( resolve, reject ) {
			...
			for ( var i = 0; i < sessionList.length; i++ ) {
				...
				sessionReportFunc.push ( aggregate.getSessionsReport ( singleSession.creationDate, singleSession.memberId, sessionId ) );
			}
			// get report info from getSessionsReport aggregation
			Q.all ( sessionReportFunc ).then ( function ( tasksResult ) {
				...
				for ( var n = 0; n < tasksResult.length; n++ ) {
					...
					for ( var k = 0; k < sessionReportList.length; k++ ) {
						...
						res.download = utility.toMByte ( sessionReport.download.value ).toFixed ( 0 )
						res.upload = utility.toMByte ( sessionReport.upload.value ).toFixed ( 0 )
						res.sessionTime = (( new Date ().getTime () - sessionCreationDate) / 60000 ).toFixed ( 0 )
					}
					report.push ( res )
				}
