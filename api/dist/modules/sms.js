        ...
        if (mobilesList && message) {
            ...
            if (businessId !== 'service') {
                Business.findById(businessId)
                    ...
                    var token10 = business.smsSignature ||
                        business.title ||
                        process.env.SMS_SIGNATURE;
                    SystemConfig.isLocal()
                        .then(function (isALocal) {
                        if (isALocal) {
                            sendMessage(mobile, token1, token2, token3, token10, template, businessId)
                                .then(function () {
                                return resolve();
                            })
                                .fail(function (error) {
    ...
    return Q.Promise(function (resolve, reject) {
        getSmsApiKey()
            ...
            if (SMS_API_KEY && SMS_API_KEY !== '-') {
                kavehnegar
                    .sendMessageToKavehnegar(SMS_API_KEY, mobile, token1, token2, token3, token10, template)
                    .then(function (cost) {
                ...
                auth
                    ...
                    needle.post(LICENSE_SERVER_SMS_API_URL.replace('{token}', authResult.token), data, function (error, result, body) {
                        if (error) {
        ...
        getSmsApiKey()
            ...
            if (SMS_API_KEY) {
                ...
                auth
                    ...
                    needle.post(LICENSE_SERVER_GROUP_SMS_API_URL.replace('{token}', authResult.token), data, { json: true }, function (error, result, body) {
                        if (error) {
