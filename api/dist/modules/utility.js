...
exports.pass2sha = function (password, salt) {
    ...
    return shaObj.getHMAC(salt.toString(), this.options.inputType, this.options.encription, this.options.outputType);
};
...
exports.sendMessage = function (error, options, moduleName) {
    ...
    log.debug(' ################ Sending error to sentry ################ ', error);
    Raven.setContext({
        user: {
    ...
    Raven.captureException(error, function (sendErr) {
        if (sendErr) {
            log.debug(' #######################  Failed to send captured exception to Sentry ########################  ', sendErr);
            return;
        }
    ...
    return Q.promise(function (resolve, reject) {
        ...
        serialize.on('end', function () {
            needle.post(ELASTIC_BULK, bulkDocs, function (error, response) {
                ...
                log.debug('@ELASTIC_BULK_INSERT#####>>>>  docs inserted to elastic: ', docs.length);
                return resolve();
            });
        });
