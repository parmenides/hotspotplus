...
module.exports = function ( app ) {
	...
	router.post ( '/api/config/systemConfig', function ( req, res ) {
		...
		SystemConfig.getConfig ().then ( function ( systemConfig ) {
			...
			systemConfig.sentryUrl = config.SENTRY_DASHBOARD_URL || config.DEFAULT_SENTRY_URL;
	...
	router.post ( "/api/radius/loadThemeConfig", function ( req, res ) {
		...
		try {
			...
			log.debug ( parsedUrl );
			if ( !parsedUrl.query.loginurl && !parsedUrl.query.actionurl && !parsedUrl.query.nasId && !parsedUrl.query.nasid && !parsedUrl.query.staticnasid ) {
			...
			if ( parsedUrl.query && parsedUrl.query.loginurl ) {
				...
				responseQuery.signInUrl = "http://" + responseQuery.uamip + ":" + responseQuery.uamport + "/login";
				...
				responseQuery.signInUrl = "http://" + responseQuery.uamip + ":" + responseQuery.uamport + "/www/login.chi";
				...
				return res.status ( 500 ).json ( { "error": "failed to extract required params" } );
			}

			Nas.loadThemeConfigById ( responseQuery.nasId, function ( error, themeConfig ) {
				if ( error ) {
					log.error ( error );
				...
				Member.loadMemberCredentialsByMac ( responseQuery.mac, themeConfig.businessId ).then ( function ( credentials ) {
