...
var log = logger.createLogger ( "license-server", process.env.LOG_DIR );
...
module.exports = function ( app ) {
  ...
  router.post ( "/api/radius/config", function ( req, res ) {
    ...
    log.debug ( data );
    License.findOne ( { "where": { "serial": receivedLicense.serial } }, function ( error, license ) {
      if ( error ) {
        //deny
        log.debug ( error );
      ...
      LicenseAudit.create ( {
        ...
        return res.status ( 200 ).send ( utility.encrypt ( JSON.stringify ( res ), license.serial ) );
      } );
    } );
