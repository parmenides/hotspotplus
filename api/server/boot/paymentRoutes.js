...
module.exports = function ( app ) {
	...
	router.post ( '/api/payment/hotspot/return', function ( req, res ) {
		...
		Member.verifySubscriptionPayment ( invoiceId, username, password, host, nasId, memberId, businessId ).then ( function ( result ) {
	...
	router.post ( '/api/payment/business/return', function ( req, res ) {
		...
		var invoiceId = req.query.invoiceId;
		Business.verifyBuyPackage ( invoiceId ).then ( function ( result ) {
			var url = result.returnUrl;
			var code = result.code;
			return res.status ( code ).redirect ( url )
		} ).fail ( function ( error ) {
	...
	router.post ( '/api/payment/external/return', function ( req, res ) {
		...
		var invoiceId = req.query.invoiceId;
		Invoice.verifyExternalInvoice( invoiceId ).then ( function ( result ) {
			var url = result.returnUrl;
			var code = result.code;
			return res.status ( code ).redirect ( url )
		} ).fail ( function ( error ) {
	...
	router.post ( '/api/payment/charge/return', function ( req, res ) {
		var Business = app.models.Business;
		var invoiceId = req.query.invoiceId;
		Business.verifyBuyCredit ( invoiceId ).then ( function ( result ) {
			var url = result.returnUrl;
			var code = result.code;
			return res.status ( code ).redirect ( url )
		} ).fail ( function ( error ) {
	...
	router.post ( '/api/payment/member/return', function ( req, res ) {
		var Member = app.models.Member;
		var invoiceId = req.query.invoiceId;
		Member.verifyPayment ( invoiceId ).then ( function ( result ) {
			var url = result.returnUrl;
			var code = result.code;
			return res.status ( code ).redirect ( url )
		} ).fail ( function ( error ) {
