...
module.exports = function ( app ) {
	...
	router.post ( '/api/radius/authorize/:nasIp', function ( req, res ) {
		...
		RadiusAdaptor.RadiusMessage ( req.body ).then ( function ( AccessRequest ) {
			AccessRequest.setNasIp ( req.params.nasIp );
			Member.authorize ( AccessRequest ).then ( function ( RadiusResponse ) {
	...
	router.post ( '/api/radius/post-auth', function ( req, res ) {
		...
		var Member = app.models.Member
		RadiusAdaptor.RadiusMessage ( req.body ).then ( function ( AccessRequest ) {
			Member.postAuth ( AccessRequest ).then ( function ( RadiusResponse ) {
				log.debug ( 'SENDING RESPONSE' )
				log.debug ( RadiusResponse.getMessage () )
				return res.status ( RadiusResponse.getCode () ).json ( RadiusResponse.getMessage () )
			} ).fail ( function ( RadiusResponse ) {
	...
	router.post ( '/api/radius/accounting/:nasIp', function ( req, res ) {
		...
		var Member = app.models.Member
		RadiusAdaptor.RadiusMessage ( req.body ).then ( function ( AccountingMessage ) {
			AccountingMessage.setNasIp ( req.params.nasIp );
			Member.accounting ( AccountingMessage ).then ( function ( RadiusResponse ) {
	...
	router.get ( '/api/radius/downloadScripts/:nasName', function ( req, res ) {
		...
		Nas.getMikrotikScripts ( bizId, nasId ).then ( function ( scriptsZippedFiles ) {
