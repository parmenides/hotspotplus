		...
		return Q.Promise ( function ( resolve, reject ) {
			redisClient.get ( "SystemConfig", function ( error, systemConfig ) {
				...
				try {
					...
					process.env.CALCULATED_WEB_APP_ADDRESS = systemConfig.webAppAddress || process.env.EXTRACTED_WEB_APP_ADDRESS;
					process.env.CALCULATED_EXTERNAL_API_ADDRESS = systemConfig.externalApiAddress || process.env.EXTRACTED_EXTERNAL_API_ADDRESS;
					process.env.CALCULATED_HOTSPOT_ADDRESS = systemConfig.hotspotAddress || process.env.EXTRACTED_HOTSPOT_ADDRESS;
					//set env
					process.env.DROPBOX_APP_SECRET = systemConfig.dropBoxAppSecret;
					process.env.DROPBOX_APP_KEY = systemConfig.dropBoxAppKey;

					redisClient.get ( 'numberOfAllowedBusiness', function ( error, numberOfAllowedBusiness ) {
						if ( error ) {
							log.error ( error );
							return reject ( error );
						}
						systemConfig.numberOfAllowedBusiness = Number ( numberOfAllowedBusiness ) || 1;
