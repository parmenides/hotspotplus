...
var mikrotik_hotspot_script_path = path.join ( __dirname, '/routers/mikrotik/html' );
...
var DEFAULT_ACCOUNTING_UPDATE_INTERVAL_SECONDS = process.env.DEFAULT_ACCOUNTING_UPDATE_INTERVAL_SECONDS || 600;
var FAST_ACCOUNTING_UPDATE_INTERVAL_SECONDS = process.env.FAST_ACCOUNTING_UPDATE_INTERVAL_SECONDS || 600;
...
module.exports = {
	...
	KAVEHNEGAR_ADD_CUSTOMER:                         "http://api.kavenegar.com/v1/" + SMS_API_KEY + "/client/add.json",
	KAVEHNEGAR_LOAD_CUSTOMER:                        "http://api.kavenegar.com/v1/" + SMS_API_KEY + "/client/fetch.json",
	...
	'hotspotCredentialMessage':                      ' شما میتوانید با نام کاربری {username} و رمز عبور {password} به اینترنت متصل شوید و یا برای اتصال خودکار بر روی لینک زیر کلیک کنید.',
	'HOTSPOT_HOME_URL':                              'http://msn.com',
	'DROPBOX_AUTHORISE_URL':                         'https://www.dropbox.com/1/oauth2/authorize?client_id={0}&response_type=code&redirect_uri={1}&state={2}',
	...
	'CONFIG_SERVER_CHARGE_SMS':                      CONFIG_SERVER_URL + '/Sms/buySmsCredit?access_token={token}',
	'CONFIG_SERVER_BUY_PACKAGE':                     CONFIG_SERVER_URL + '/Licenses/buyLocalPackage?access_token={token}',
	'CONFIG_SERVER_LOCAL_MODULES':                   CONFIG_SERVER_URL + '/Licenses/getPackages',
	'CONFIG_SERVER_LOCAL_CHARGE':                    CONFIG_SERVER_URL + '/Sms/getLicenseBalance?access_token={token}',
	'CONFIG_SERVER_NEW_LICENSE':                     CONFIG_SERVER_URL + '/Licenses/registerPublicLicense',
	...
	TSHARK_TRANSFORM_COMMAND:                        'tshark -r {1}  -T fields -e wlan.sa -e wlan.da -e frame.time_epoch -e radiotap.dbm_antsignal -e  wlan_mgt.ssid -E header=y -E separator=,   > {0}',
	AGGREGATE:                                       {
		...
		QUARTER_WINTER_MILLISECONDS:      2 * 30 * 24 * 60 * 60 * 1000 + 29 * 24 * 60 * 60 * 1000,
		YEAR_MILLISECONDS:                6 * 31 * 24 * 60 * 60 * 1000 + 5 * 30 * 24 * 60 * 60 * 1000 + 29 * 24 * 60 * 60 * 1000,
		YEAR_LEAP_MILLISECONDS:           6 * 31 * 24 * 60 * 60 * 1000 + 6 * 30 * 24 * 60 * 60 * 1000,
		WEEK_DAYS:                        7,
		MONTH_DAYS:                       30
	},
	...
	'ENGLISH_SMS_COST':                           46,
	'PERSIAN_SMS_COST':                           19,
	'SYSLOG_REDIS_CHANNEL':                       process.env.SYSLOG_REDIS_CHANNEL,
	'NETFLOW_REDIS_CHANNEL':                      process.env.NETFLOW_REDIS_CHANNEL,
	...
	'ELASTIC_VISIT_REPORT_MAIN_CONTEXT':          elasticURL + process.env.ELASTIC_INDEX_PREFIX + 'visit',
	'ELASTIC_VISIT_REPORT':                       elasticURL + process.env.ELASTIC_INDEX_PREFIX + 'visit/report',
	'ELASTIC_CHARGE_MAIN_CONTEXT':                elasticURL + process.env.ELASTIC_INDEX_PREFIX + 'charge',
	'ELASTIC_CHARGE_SEARCH':                      elasticURL + process.env.ELASTIC_INDEX_PREFIX + 'charge/charge/_search',
	'ELASTIC_ACCOUNTING_MAIN_CONTEXT':            elasticURL + process.env.ELASTIC_INDEX_PREFIX + 'accounting',
	'ELASTIC_ACCOUNTING_USAGE':                   elasticURL + process.env.ELASTIC_INDEX_PREFIX + 'accounting/usagereport',
	'ELASTIC_ACCOUNTING_USAGE_SEARCH':            elasticURL + process.env.ELASTIC_INDEX_PREFIX + 'accounting/usagereport/{0}{1}',
	'ELASTIC_SYSLOG_MAIN_CONTEXT':                elasticURL + process.env.ELASTIC_INDEX_PREFIX + 'syslog',
	'ELASTIC_SYSLOG_REPORT':                      elasticURL + process.env.ELASTIC_INDEX_PREFIX + 'syslog/report',
	'ELASTIC_NETFLOW_MAIN_CONTEXT':               elasticURL + process.env.ELASTIC_INDEX_PREFIX + 'netflow',
	'ELASTIC_NETFLOW_REPORT':                     elasticURL + process.env.ELASTIC_INDEX_PREFIX + 'netflow/report',
	'ELASTIC_TEST_PATH':                          elasticURL + process.env.ELASTIC_INDEX_PREFIX + 'test/test/',
	'ELASTIC_BULK':                               elasticURL + process.env.ELASTIC_INDEX_PREFIX + '{0}/{1}/_bulk',
	'ELASTIC_DELETE_BY_QUERY':                    elasticURL + process.env.ELASTIC_INDEX_PREFIX + '{0}/{1}/_delete_by_query',
	'VOUCHER_TEMPLATE_PATH':                      templatesPath + "memberPrintTemplate.dust.html",
	'SCRIPTS':                                    {
		'MIKROTIK_DYNAMIC_IP_SCRIPT': setup_script_path + 'mikrotik_dynamicip_setup.sh',
		'MIKROTIK_SETUP_SCRIPT':      setup_script_path + 'mikrotik_setup.sh',
		'MIKROTIK_HOTSPOT_PAGES':     [
			{ path: mikrotik_hotspot_script_path + '/alogin.html', name: 'alogin.html' },
			{ path: mikrotik_hotspot_script_path + '/fstatus.html', name: 'fstatus.html' },
			{ path: mikrotik_hotspot_script_path + '/login.html', name: 'login.html' },
			{ path: mikrotik_hotspot_script_path + '/logout.html', name: 'logout.html' },
			{ path: mikrotik_hotspot_script_path + '/redirect.html', name: 'redirect.html' },
			{ path: mikrotik_hotspot_script_path + '/rlogin.html', name: 'rlogin.html' },
			{ path: mikrotik_hotspot_script_path + '/rstatus.html', name: 'rstatus.html' },
			{ path: mikrotik_hotspot_script_path + '/status.html', name: 'status.html' }
		]
	},
	...
	'DEFAULT_MEMBER_SESSION_EXPIRE_IN_SECONDS':   DEFAULT_ACCOUNTING_UPDATE_INTERVAL_SECONDS + 10,
	...
	'SERVICES':                                   {
		...
		"features":                  [
			...
			},
			{
				...
				"desc":         "با استفاده از تعریف پلن‌های ماهانه و روزانه میتوان بر روی حجم، مدت زمان، و سرعت کاربر محدودیت اعمال کرد",
				"type":         "boolean",
				"monthly":      true,
				"semiAnnually": true,
				"annually":     true
			},
			{
				...
				"semiAnnually": true,
				"type":         "boolean",
				"desc":         "کاربران میتوانند با نام کاربری و رمز عبور خود به پنل لاگین کنند و میزان مصرف خود را مشاهده کنند."
			},
			{
				"title":        "تغییر لوگو و فرم عضویت در صفحات هات‌اسپات",
				"desc":         "با استفاده از این امکان میتوانید لوگوی صفحه‌های هات‌اسپات را از طریق پنل کاربری به لوگوی مورد نظر خودتان تغییر دهید و فرم عضویت کاربران در هات اسپات را تغییر دهید و اطلاعات مورد نظر خود را مانند تاریخ تولد از کاربر دریافت کنید.",
				"monthly":      true,
				"semiAnnually": true,
				"annually":     true,
				"type":         "boolean"
			},
			{
				...
				"desc":         "این پیامک‌ها به دلیل ارسال از طریق خطوط خدماتی، فیلتر نمی‌شوند و تا ۹۹٪ مواقع خیلی سریع به گوشی کاربر می‌رسند.",
				"monthly":      true,
				"semiAnnually": true,
				"annually":     true,
				"type":         "boolean"
			...
			},
			{
				...
				"annually":     true,
				"type":         "boolean",
				"desc":         "با استفاده از این امکان می‌توانید پروفایل مشتریان را دانلود کنید یا برای آنها پیامک‌های تبلیغاتی ارسال کنید."
			},
			{
				...
				"annually":     true,
				"type":         "boolean",
				"desc":         "با استفاده از این امکان میتوانید متن انتهای پیامک (از طرف...) را به نام مورد نظر خودتان تغییر دهید."
			}
		]
	},
	RESELLERS_TARIFFS:                            [
		...
		{ from: 6, to: 100, free: 0, silver: 20000, gold: 30000, whiteLabel: 10000 }
	],
	DEFAULT_HOTSPOT_HELP:                         "برای اتصال به اینترنت، به شبکه وای فای متصل شوید. سپس با مرورگر کروم به سایت wifi.ir وارد شوید. پس از باز شدن صفحه با نام کاربری و رمز زیر به هات اسپات لاگین کنید.",
	...
	'BUSINESS_PAYMENT_RETURN_URL': function () {
		return process.env.CALCULATED_EXTERNAL_API_ADDRESS + '/api/payment/business/return?{0}={1}';
	},
	/**
	 * @return {string}
	 */
	'EXTERNAL_PAYMENT_RETURN_URL': function () {
		return process.env.CALCULATED_EXTERNAL_API_ADDRESS + '/api/payment/external/return?{0}={1}';
	},
	/**
	 * @return {string}
	 */
	'CHARGE_PAYMENT_RETURN_URL':   function () {
		return process.env.CALCULATED_EXTERNAL_API_ADDRESS + '/api/payment/charge/return?{0}={1}';
	},
	/**
	 * @return {string}
	 */
	'LOCAL_PAYMENT_RETURN_URL':    function () {
		return process.env.CALCULATED_EXTERNAL_API_ADDRESS + '/api/payment/local/return?{0}={1}';
	},
	/**
	 * @return {string}
	 */
	'MEMBER_PAYMENT_RETURN_URL':   function () {
		return process.env.CALCULATED_EXTERNAL_API_ADDRESS + '/api/payment/member/return?{0}={1}';
	},
	/**
	 * @return {string}
	 */
	...
	'BUY_LOCAL_SMS_CHARGE_RETURN':    function () {
		return process.env.CALCULATED_WEB_APP_ADDRESS + mainPath + '#/app/loading?payed={0}&desc={1}&license_updated=true'
	},
	/**
	 * @return {string}
	 */
	...
	'BUSINESS_PAYMENT_RESULT_URL':    function () {
		return process.env.CALCULATED_WEB_APP_ADDRESS + mainPath + '#/app/loading?payed={0}{1}'
	},
	/**
	 * @return {string}
	 */
	'LOCAL_PAYMENT_RESULT_URL':       function () {
		return process.env.CALCULATED_WEB_APP_ADDRESS + mainPath + '#/access/public/local'
	},
	/**
	 * @return {string}
	 */
	...
	'DROPBOX_AUTHORISE_RESULT_URL':   function () {
		return process.env.CALCULATED_WEB_APP_ADDRESS + mainPath + '#/app/loading?dropbox={0}{1}'
	},
	/**
	 * @return {string}
	 */
	'HOTSPOT_PAYMENT_RETURN_URL':     function () {
		return process.env.CALCULATED_HOTSPOT_ADDRESS + '/api/payment/hotspot/return?{0}={1}&password={password}&username={username}&businessId={businessId}&nasId={nasId}&host={host}&memberId={memberId}';
	},
	/**
	 * @return {string}
	 */
	...
	'HOTSPOT_VERIFICATION_URL':       function () {
		return process.env.CALCULATED_HOTSPOT_ADDRESS + '/#!/home.html?businessId={businessId}&nasId={nasId}&host={host}&verificationCode={verificationCode}&memberId={memberId}';
	},
	/**
	 * @return {string}
	 */
	'HOTSPOT_SIGNIN_URL':             function () {
		return process.env.CALCULATED_HOTSPOT_ADDRESS + '/#!/home.html?businessId={businessId}&nasId={nasId}&host={host}&username={username}&password={password}&memberId={memberId}';
	},
	/**
	 * @return {string}
	 */
