...
var dataSource = {
	...
	'url':       'mongodb://' + process.env.MONGO_IP + ':27017/' + process.env.MONGO_DB_NAME + '?w=1&j=true',
	'name':      'mongo'
};
...
app.use ( function ( req, res, next ) {
	if ( req.path.indexOf ( '/api' ) !== -1 ) {
		if ( req.path.indexOf ( '/api/radius/loadThemeConfig' ) !== -1 ) {
			process.env.EXTRACTED_HOTSPOT_ADDRESS = req.protocol + "://" + req.get ( 'Host' );
		} else {
			process.env.EXTRACTED_EXTERNAL_API_ADDRESS = req.protocol + "://" + req.get ( 'Host' );
