...
var redisLicenseRenew = redis.createClient ( process.env.REDIS_PORT, process.env.REDIS_IP );
var redisLicenseLoaded = redis.createClient ( process.env.REDIS_PORT, process.env.REDIS_IP );
...
const pkString = "-----BEGIN RSA PUBLIC KEY-----\n" +
	...
	return Q.Promise ( function ( resolve, reject ) {
		...
		fs.readFile ( lcPath, 'utf8', function ( error, lcFileContent ) {
			...
			try {
				...
				utility.writeStringToFile ( decryptedLc ).then ( function ( filePath ) {
					fs.unlink ( lcPath );
...
function validateLicense () {
	return Q.Promise ( function ( resolve, reject ) {
		...
		loadLicense ().then ( function ( licensePath ) {
			utility.writeStringToFile ( pkString ).then ( function ( publicKeyPath ) {
				try {
					var lcData = lc.parse ( {
					...
					if ( lcData.valid === true ) {
						...
						if ( new Date ( lcData.data.expiresAt ).isBefore ( new Date () ) ) {
							...
							for ( var i in lcData.data ) {
								if ( i !== 'scripts' && i !== 'systemConfig' ) {
									(function ( j ) {
										...
										redisClient.set ( j, lcData.data[ j ], "EX", KEYS_EXPIRES, function ( error ) {
											if ( error ) {
												log.error ( error );
												return reject ();
											}
							...
							redisClient.set ( "SystemConfig", lcData.data.systemConfig, "EX", KEYS_EXPIRES, function ( error ) {
								if ( error ) {
									log.error ( error );
									return reject ();
								}
								var scripts = JSON.parse ( publicKey.decryptPublic ( lcData.data.scripts, 'utf8' ) );
								var methods = scripts.scripts;
								var methodNames = [];
								for ( var i in  methods ) {
									methodNames.push ( i );
									redisClient.set ( 'sc_' + i, methods[ i ], "EX", KEYS_EXPIRES, function ( error ) {
										if ( error ) {
											log.error ( error );
											return reject ();
										}
								...
								redisClient.set ( 'methodNames', JSON.stringify ( methodNames ), 'EX', KEYS_EXPIRES, function ( error ) {
									if ( error ) {
										log.error ( error );
										return reject ();
									}
...
function renewLicense () {
	return Q.Promise ( function ( resolve, reject ) {
		...
		utility.getSystemUuid ( SYSTEM_ID_PATH ).then ( function ( systemUuid ) {
			log.debug ( systemUuid );
			log.debug ( config.LICENSE_LOGIN, systemUuid, config.PASSWORD_PREFIX + utility.md5 ( systemUuid ) );
			authUtility.loginToConfigServer ( config.LICENSE_LOGIN, systemUuid, config.PASSWORD_PREFIX + utility.md5 ( systemUuid ) ).then ( function ( authResult ) {
				...
				needle.get ( config.DOWNLOAD_LICENSE.replace ( '{token}', token ).replace ( '{systemUuid}', systemUuid ), {
