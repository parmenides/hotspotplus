	...
	return Q.Promise ( function ( resolve, reject ) {
		needle.get ( "http://myip.dnsomatic.com/", function ( error, response ) {
			...
			utility.getSystemUuid ( config.SYSTEM_ID_PATH ).then ( function ( systemUuid ) {
				auth.loginToLicenseServer ( config.LICENSE_LOGIN ).then ( function ( authResult ) {
					var token = authResult.token;
					needle.post ( config.CONFIG_SERVER_PING.replace ( "{token}", token ), {
						ip:         ip,
						systemUuid: systemUuid
					}, { rejectUnauthorized: true, json: true }, function ( error, result, body ) {
						if ( error ) {
							log.error ( error );
							return reject ();
						}
