...
app.controller('businessList', [
  ...
  function(
      ...
      $uibModal.open({
        ...
        controller: [
          ...
          function($scope, $uibModalInstance, Business) {
            ...
            $scope.save = function() {
              Business.resetPasswordByAdmin($scope.data).$promise.then(
                function(result) {
                  $log.debug(result);
    ...
    $scope.deleteBusiness = function(row) {
      genericService.showConfirmDialog({
        ...
        yesCallback: function() {
          ...
          Business.destroyById({ id: businessId }).$promise.then(
            function(res) {
            ...
            controller: [
              ...
              function($scope, $uibModalInstance) {
                if (business.services && business.services.duration) {
                  business.services.durationInMonths =
                    business.services.duration;
                }
                $scope.serviceOption = {};
      ...
      Business.findById({ id: businessId }).$promise.then(
        function(business) {
          $uibModal.open({
            ...
            scope: $scope,
            templateUrl:
              PREFIX + 'app/business/tpl/assignBusinessToResellerForm.html',
            controller: [
              '$scope',
              '$uibModalInstance',
              function($scope, $uibModalInstance) {
                ...
                $scope.save = function() {
                  Reseller.assignBusinessToReseller($scope.data).$promise.then(
                    function() {
                      appMessenger.showSuccess('business.updateSuccessFull');
                      getPage();
                      $uibModalInstance.close();
    ...
    $scope.showInvoices = function(row) {
      ...
      Business.invoices(options).$promise.then(
        function(invoices) {
          $uibModal.open({
            backdrop: true,
            animation: true,
    ...
    $scope.searchBusiness = function() {
      ...
      if ($scope.businessMobile) {
        var mobilePattern =
          '/' + englishNumberFilter($scope.businessMobile) + '/i';
    ...
    $scope.$watchGroup(['businessId', 'businessMobile'], function(
      ...
      if (
        newValues != oldValues &&
        newValues[0].length == 0 &&
        newValues[1].length == 0
      ) {
        getPage();
      }
    });
    ...
    var getPage = function(inputFilter) {
      ...
      Business.count({ where: inputFilter }).$promise.then(
        function(result) {
            ...
            Business.getBalance({ businessId: business.id }).$promise.then(
              function(res) {
                ...
                if (business.resellerId) {
                  Reseller.findById({ id: business.resellerId }).$promise.then(
                    function(reseller) {
                      $scope.gridOptions.data[index].resellerTitle =
                        reseller.title;
                    },
                    function(error) {
                      $log.error(error);
