    ...
    return {
      ...
      controller: function($scope) {
        ...
        if (!Session.member.businessId || !Session.member.id) {
          ...
          Member.getDailyUsage(options).$promise.then(
            function(data) {
              var persianDate = [];
              if (
                data.date.length > 0 &&
                data.date.length == data.download.length &&
                data.date.length == data.upload.length
              ) {
                var date = data.date;
                    ...
                    yAxes: [
                      {
                        ...
                        scaleLabel: {
                          display: true,
                          labelString:
                            translateFilter('dashboard.yAxesLabelAmount') +
                            ' ( ' +
                            translateFilter('dashboard.Mbps') +
                            ' )',
                          fontColor: '#333'
