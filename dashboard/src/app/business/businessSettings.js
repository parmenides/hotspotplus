...
app.controller ( 'businessSettings', [ '$http', '$scope', '$log', '$state', 'Business', 'appMessenger', 'genericService', 'Session', 'credit', 'Charge', 'englishNumberFilter', 'translateNumberFilter', "Nas", 'AclService',
	function ( $http, $scope, $log, $state, Business, appMessenger, genericService, Session, credit, Charge, englishNumberFilter, translateNumberFilter, Nas, AclService ) {
		...
		$scope.RADIUS_CONFIG = {};
		Nas.loadRadiusInfo ().$promise.then ( function ( res ) {
		...
		Business.findById ( { id: businessId } ).$promise.then ( function ( business ) {
			...
			$scope.business.groupMemberHelps = $scope.business.groupMemberHelps || {};
		...
		$http.get ( 'app/business/timeZones.json' )
			.then ( function ( res ) {
				$scope.timeZones = res.data
			} )

		$scope.$watch ( 'business.autoAssignInternetPlan', function ( newValue, oldValue ) {
			if ( newValue == true && $scope.internetPlans && $scope.internetPlans.length == 0 ) {
		...
		$scope.save = function () {
			...
			Business.prototype$updateAttributes ( { id: businessId }, $scope.business ).$promise.then ( function ( res ) {
		...
		$scope.changePassword = function () {
			genericService.showPasswordForm ( {
				...
				saveCallback:   function ( currentPassword, password ) {
					...
					Business.login ( credential ).$promise.then ( function ( result ) {
						Business.prototype$updateAttributes ( { id: businessId }, { password: password } ).$promise.then ( function ( res ) {
		...
		$scope.dropboxAccess = function(){
			Business.dropBoxAuthorization ( { id: businessId } ).$promise.then ( function ( dropBox ) {
				if(dropBox.code == 302 ){
