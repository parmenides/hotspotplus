// Campaign controller
app.controller ( 'campaignList', [ '$scope', '$log', 'Business', 'Campaign', 'genericService', 'uiGridConstants', '$uibModal', 'PREFIX', 'Session', 'appMessenger',
	function ( $scope, $log, Business, Campaign, genericService, uiGridConstants, $uibModal, PREFIX, Session, appMessenger ) {
		...
		$scope.removeCampaign = function ( campaign ) {
			genericService.showConfirmDialog ( {
				...
				yesCallback: function () {
					...
					Business.campaigns.destroyById ( { id: businessId }, { fk: campaignId } ).$promise.then ( function ( res ) {
						getPage ();
		...
		$scope.openCampaignModal = function ( campaign ) {
			...
			$uibModal.open ( {
				...
				controller:    [ '$scope', '$uibModalInstance', 'campaignService', function ( $scope, $uibModalInstance, campaignService ) {
					...
					$scope.$watch ( 'campaign.type', function ( newValue, oldValue ) {
						...
						if ( newValue != oldValue && newValue != 'sendBulkMessage' ) {
							$scope.campaign.end = new Date ( $scope.campaign.start.getTime () + 7 * 24 * 60 * 60 * 1000 );
						}
					} );
					// --> for calendar bug
					...
					$scope.save = function () {
						...
						if ( !validationError ) {
							if ( !$scope.campaign.id ) {
								Business.campaigns.create ( { id: businessId }, $scope.campaign ).$promise.then ( function ( campaign ) {
									getPage ();
									appMessenger.showSuccess ( 'campaign.createSuccessFull' );
									switch ( $scope.campaign.type ) {
										case 'sendBulkMessage':
											Campaign.sendBulkMessages ( { campaignId: campaign.id } ).$promise.then ( function ( res ) {
												$uibModalInstance.close ();
												appMessenger.showSuccess ( 'campaign.sendBulkMessageSuccessFull' );
											}, function ( err ) {
												appMessenger.showError ( 'campaign.sendBulkMessageUnSuccessFull' );
											} );
											break;
										default:
											$uibModalInstance.close ();
								...
								}, $scope.campaign ).$promise.then ( function ( res ) {
									getPage ();
		...
		$scope.changeCampaignStatus = function ( item ) {
			var campaignId = item.id;
			Business.campaigns.findById ( { id: businessId, fk: campaignId } ).$promise.then ( function ( campaign ) {
				$uibModal.open ( {
					backdrop:      true,
					animation:     true,
					keyboard:      true,
						...
						$scope.yes = function () {
							...
							Business.campaigns.updateById ( {
								...
								if ( active ) {
									...
									appMessenger.showSuccess ( 'campaign.disabledSuccessFull' )
								}
								getPage ()
							}, function ( err ) {
								if ( active ) {
									...
									appMessenger.showError ( 'campaign.disabledUnSuccessFull' )
								}
							} );
							$uibModalInstance.close ()
						}
		...
		var getPage = function () {
			...
			Business.campaigns.count ( { id: businessId } ).$promise.then ( function ( result ) {
				$scope.gridOptions.totalItems = result.count;
				$scope.paginationOptions.totalItems = result.count;
			}, function ( error ) {
				$log.error ( error );
			} );
			Business.campaigns ( options ).$promise.then ( function ( campaigns ) {
				...
				angular.forEach ( campaigns, function ( campaign ) {
					if ( (campaign.active) && (campaign.start <= today) && (today <= campaign.end) ) {
