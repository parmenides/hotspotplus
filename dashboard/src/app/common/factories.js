...
app.factory ( 'authorization', [ 'Session', function ( Session ) {
	var authorize = function ( loginRequired, requiredPermissions, permissionCheckType ) {
		var result = 'authorized',
			user = Session.business || Session.user || Session.reseller || Session.member,
			loweredPermissions = [],
			hasPermission = true,
			permission, i
...
} ] ).factory ( 'Session', [ 'Business', 'Member', 'Reseller', function ( Business, Member, Reseller ) {
	var props = [ 'business', 'roles', 'user', 'userType', 'reseller', 'member' ]
	...
	function Session () {
		...
		if ( window.location.href && window.location.href.indexOf ( "public" ) == -1 ) {
	...
	Session.prototype.setSession = function ( data ) {
		...
		Raven.setUserContext ( { id: theUser.id, email: theUser.username + "@hotspotplus.ir" } );
		this.save ()
	}
