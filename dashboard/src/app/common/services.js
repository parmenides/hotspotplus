 ...
 */
app.service ( 'genericService', [ '$uibModal', '$log', '$q', '$location', 'PREFIX', function ( $uibModal, $log, $q, $location, PREFIX ) {
	this.showPasswordForm = function ( options ) {
		$uibModal.open ( {
			...
			controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
				...
				$scope.save = function () {
					$uibModalInstance.close ()
					options.saveCallback && options.saveCallback ( $scope.currentPassword, $scope.password )
				}
			} ]
		} )
	}
...
} ] ).service ( 'crudUtils', [ '$log', 'translateFilter', function ( $log, translateFilter ) {
...
} ] ).service ( 'appMessenger', [ 'toaster', 'translateFilter', function ( toaster, translateFilter ) {
	this.showSuccess = function ( message, title ) {
		if ( title ) {
			title = 'common.success'
		}
		toaster.pop ( 'success', translateFilter ( title ), translateFilter ( message ) )
	}
	...
	this.showWarning = function ( message, title ) {
		...
		toaster.pop ( 'warning', translateFilter ( title ), translateFilter ( message ) )
	}
...
} ] ).service ( 'roleService', [ '$log', '$q', 'Role', 'RoleMapping', function ( $log, $q, Role, RoleMapping ) {
	var self = this

	this.getRoles = function ( principalId ) {
		return $q ( function ( resolve, reject ) {
			RoleMapping.find ( { filter: { where: { principalType: 'USER', principalId: principalId } } } )
				.$promise.then ( function ( roleMapping ) {
				var roleIds = []
				roleMapping.forEach ( function ( rMap ) {
					roleIds.push ( rMap.roleId )
				} )
				if ( roleIds.length > 0 ) {
					self.getRoleNames ( roleIds ).then ( function ( names ) {
						resolve ( names )
					}, function ( error ) {
						reject ( error )
					} )
	...
	this.getRoleNames = function ( roleIds ) {
		return $q ( function ( resolve, reject ) {
			...
			if ( ids.length > 0 ) {
				Role.find ( q ).$promise.then ( function ( roles ) {
					var names = []
...
} ] ).service ( 'dashboardTiming', [ '$log', 'PersianDateService', function ( log, persianDate ) {
	...
	this.startOfWeek = function ( date ) {
		...
		date = new Date ( date.getTime () - dayOfWeek * DAY_MILLISECONDS ).setHours ( 0, 0, 0, 0 )
		return date
	}
	this.startOfMonth = function ( date ) {
		var dayOfMonth = persianDate.getDate ( date ) - 1
		return new Date ( date.getTime () - dayOfMonth * DAY_MILLISECONDS ).setHours ( 0, 0, 0, 0 )
	}
	this.endOfMonth = function ( date ) {
		...
		return new Date ( date.getTime () + (monthDays - dayOfMonth) * DAY_MILLISECONDS ).setHours ( 0, 0, 0, 0 )
	}
	this.advanceTime = function ( startDate, endDate ) {
		var monthDays = []
		var result = { monthDays: monthDays, startDate: startDate, endDate: endDate }
		...
		if ( daySelected > 30 ) {
			...
			while ( startDateYear <= endDateYear && startDateMonth <= endDateMonth ) {
				monthDays[ counter ] = persianDate.persianMonthDays ( startDateYear, startDateMonth )
			...
			result = { monthDays: monthDays, startDate: startDate, endDate: endDate }
		}
		return result
	}
	this.getMonthTiming = function ( month ) {
		...
		result.stratDate = new Date ( today.getTime () - (days * DAY_MILLISECONDS) ).setHours ( 0, 0, 0, 0 )
		result.endDate = new Date ( today.getTime () - ( (days - monthDays + 1) * DAY_MILLISECONDS) ).setHours ( 23, 59, 59, 0 )
		return result
	}
} ] ).service ( 'credit', [ '$log', '$uibModal', 'PREFIX', 'englishNumberFilter', 'Business', '$window', 'appMessenger', function ( $log, $uibModal, PREFIX, englishNumberFilter, Business, $window, appMessenger ) {
	this.showIncreaseCreditForm = function ( options ) {
		$uibModal.open ( {
			...
			controller:    [ '$scope', '$uibModalInstance', 'Business', '$state', '$stateParams', function ( $scope, $uibModalInstance, Business ) {
				$scope.options = options || {}
				...
				$scope.payment = function () {
					...
					if ( $scope.options.chargedBy == 'business' ) {
						$scope.bankWait = true
						Business.buyCredit ( paymentOptions ).$promise.then ( function ( res ) {
							if ( res.url ) {
								$window.location.href = res.url
								var message = 'business.chargedSuccessful'
								options.saveCallback && options.saveCallback ( false, message )
							} else {
								var errorMessage = 'error.generalError'
								options.saveCallback && options.saveCallback ( true, errorMessage )
							}
						}, function ( err ) {
							var errorMessage = 'error.generalError'
							options.saveCallback && options.saveCallback ( true, errorMessage )
						...
						Business.adminChargeCredit ( paymentOptions ).$promise.then ( function ( res ) {
							$uibModalInstance.close ()
							var message = 'business.chargedSuccessful'
							options.saveCallback && options.saveCallback ( false, message )
						}, function ( err ) {
...
} ] ).service ( 'uploadModal', [ '$log', '$uibModal', 'PREFIX', 'FileStorage', 'FileUploader', 'appMessenger', 'genericService', 'Business',
	function ( $log, $uibModal, PREFIX, Files, FileUploader, appMessenger, genericService, Business ) {
			...
			$uibModal.open ( {
				...
				controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
					...
					function getFiles ( business ) {
						Files.getFilesByBusinessId ( business )
							.$promise.then ( function ( files ) {
							$scope.fileList = files.result
						} ), function ( error ) {
							$log.error ( 'can not get files info from data source: ' + error )
					...
					$scope.removeImage = function ( fileId ) {
						genericService.showConfirmDialog ( {
							...
							yesCallback: function () {
								Business.files.destroyById ( { id: businessId }, { fk: fileId } ).$promise.then ( function ( res ) {
									...
									if ( options.themeId && options.businessThemeConfig ) {
										angular.forEach ( options.businessThemeConfig, function ( themeConfig, themeId ) {
											if ( themeConfig[ 'logo' ] && themeConfig[ 'logo' ].id && themeConfig[ 'logo' ].id == fileId ) {
												themeConfig[ 'logo' ] = {};
											...
											if ( themeConfig[ 'background' ] && themeConfig[ 'background' ].id && themeConfig[ 'background' ].id == fileId ) {
	...
	.service ( 'usernameService', [ '$log', function ( $log ) {
	...
	.service ( 'errorMessenger', [ '$log', function ( $log ) {
