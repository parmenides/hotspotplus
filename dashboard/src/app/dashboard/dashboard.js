...
app.controller ( 'dashboard', [ '$scope', '$log', 'Session', 'dashboardTiming', 'appMessenger', '$timeout', 'translateFilter', 'persianDateFilter',
	function ( $scope, $log, Session, dashboardTiming, appMessenger, $timeout, translateFilter, persianDateFilter ) {
		...
		function getFromToDate ( fromDate, endDate ) {
			if ( !fromDate ) fromDate = new Date ( new Date ().getTime () ).setHours ( 0, 0, 0, 0 )
			if ( !endDate ) endDate = new Date ().setDate ( new Date(fromDate).getDate () + 7 );
