...
app.controller ( 'internetPlanList', [ '$scope', '$state', '$log', 'translateFilter', 'Business', 'InternetPlan', 'uiGridConstants', '$http', 'genericService', '$uibModal', 'PREFIX', 'Session', 'appMessenger', 'translateNumberFilter', 'englishNumberFilter',
	function ( $scope, $state, $log, translateFilter, Business, InternetPlan, uiGridConstants, $http, genericService, $uibModal, PREFIX, Session, appMessenger, translateNumberFilter, englishNumberFilter ) {
			...
			columnDefs:               [
				{
					...
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.editPlan(row)">{{row.entity.name}}</a>'
				},
				{
				...
				},
				{
					...
					headerCellFilter: 'translate',
					cellFilter:       'translate',
					cellTemplate:     '<div class="ui-grid-cell-contents"><span ng-if="row.entity.price != 0">{{row.entity.price | translateNumber }} {{"general.toman" | translate }}</span><span ng-if="row.entity.price == 0">{{"general.free" | translate}}</span></div>'
				},
				{
					...
					cellTemplate:     '<a class="btn btn-link" ng-disabled="row.entity.price != 0" ng-click="grid.appScope.editDefaultPlanSettings()" uib-popover={{grid.appScope.selectDefaultPlanSettingsText}} popover-trigger="mouseenter"' +
				...
				},
				{
					...
					cellClass:        'center',
					headerCellClass:  'center',
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.editPlan(row)"><i class="fa fa-fw fa-pencil"></i></a>'
				},
				{
					...
					cellClass:        'center',
					headerCellClass:  'center',
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.removePlan(row)"><i class="fa fa-fw fa-trash"></i></a>'
				}
			],
		...
		$scope.selectDefaultPlanSettingsText = translateFilter ( 'help.selectDefaultPlanSettings' );
		$scope.removePlan = function ( row ) {
			genericService.showConfirmDialog ( {
				...
				yesCallback: function () {
					...
					Business.internetPlans.destroyById ( { id: businessId }, { fk: internetPlanId } ).$promise.then ( function ( res ) {
			...
			$uibModal.open ( {
				...
				controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
					...
					$scope.save = function () {
						...
						Business.internetPlans.create ( { id: businessId }, $scope.internetPlan ).$promise.then ( function ( res ) {
		...
		$scope.editPlan = function ( row ) {
			$uibModal.open ( {
				...
				controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
					...
					var internetPlanId = row.entity.id;
					Business.internetPlans.findById ( {
						id: businessId,
						fk: internetPlanId
					} ).$promise.then ( function ( internetPlan ) {
					...
					$scope.save = function () {
						$scope.internetPlan = englishInternetPlan ( $scope.internetPlan )
						Business.internetPlans.updateById ( {
							id: businessId,
							fk: internetPlanId
						}, $scope.internetPlan ).$promise.then ( function ( res ) {
		...
		$scope.editDefaultPlanSettings = function () {
			$uibModal.open ( {
				...
				backdropClick: true,
				scope:         $scope,
				templateUrl:   PREFIX + 'app/internetPlan/tpl/internetPlanSettingsForm.html',
				controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
					$scope.options = {
					...
					Business.internetPlans ( { id: businessId } ).$promise.then ( function ( internetPlans ) {
						...
						angular.forEach ( internetPlans, function ( internetPlan ) {
							if ( $scope.defaultInternetPlan && $scope.defaultInternetPlan.id && $scope.defaultInternetPlan.id == internetPlan.id ) {
								$scope.defaultInternetPlan.plan = internetPlan
							}
						} );
						if ( $scope.defaultInternetPlan && $scope.defaultInternetPlan.count == 'undefined' ) {
							$scope.defaultInternetPlan.count = translateNumberFilter ( 0 )
						}
						if ( $scope.defaultInternetPlan && $scope.defaultInternetPlan.period == 'undefined' ) {
							$scope.defaultInternetPlan.period = translateNumberFilter ( 0 )
						}
						if ( $scope.defaultInternetPlan && $scope.defaultInternetPlan.count ) {
							$scope.defaultInternetPlan.count = translateNumberFilter ( $scope.defaultInternetPlan.count )
						}
						if ( $scope.defaultInternetPlan && $scope.defaultInternetPlan.period ) {
							$scope.defaultInternetPlan.period = translateNumberFilter ( $scope.defaultInternetPlan.period )
						}
					}, function ( error ) {
						$log.error ( error )
					} )
					...
					$scope.save = function () {
						...
						if ( $scope.defaultInternetPlan && $scope.defaultInternetPlan.plan ) {
							defaultInternetPlan = {
								id:         $scope.defaultInternetPlan.plan.id,
								count:      englishNumberFilter ( $scope.defaultInternetPlan.count ) || 0,
								period:     englishNumberFilter ( $scope.defaultInternetPlan.period ) || 0,
								autoAssign: $scope.defaultInternetPlan.autoAssign
							}
						}
						Business.prototype$updateAttributes ( { id: businessId }, { defaultInternetPlan: defaultInternetPlan } ).$promise.then ( function ( res ) {
		...
		var getPage = function () {
			...
			Business.internetPlans.count ( { id: businessId } ).$promise.then ( function ( result ) {
				$scope.gridOptions.totalItems = result.count
				$scope.paginationOptions.totalItems = result.count
			}, function ( error ) {
				$log.error ( error )
			} )
			Business.internetPlans ( options ).$promise.then ( function ( internetPlans ) {
				$scope.gridOptions.data = internetPlans
				Business.findById ( { id: businessId } ).$promise.then ( function ( business ) {
					if ( business.defaultInternetPlan && business.defaultInternetPlan.id ) {
						$scope.defaultInternetPlan = business.defaultInternetPlan
						angular.forEach ( $scope.gridOptions.data, function ( internetPlan ) {
