 ...
 */
app.controller ( 'buyPlan', ['$scope', '$log', 'Session', 'appMessenger', '$uibModal', 'PREFIX', 'InternetPlan', '$window', '$state', 'Member',
	function ( $scope, $log, Session, appMessenger, $uibModal, PREFIX, InternetPlan, $window, $state, Member  ) {
		...
		$scope.showPlans = function () {
			InternetPlan.find (
				...
				$uibModal.open ( {
					...
					controller:    ['$scope', '$uibModalInstance', 'Member', function ( $scope, $uibModalInstance, Member ) {
						...
						$scope.payment = function ( planId, planPrice ) {
							...
							if ( planPrice === 0 ) {
								...
								InternetPlan.assignFreePlanToMember ( options ).$promise.then ( function ( result ) {
										if ( result.ok == true ) {
											$scope.cancel ();
										...
										$log.error ( error );
										if ( error.data.error.message == 'default plan activated too many times' ) {
											appMessenger.showError ( 'member.defaultPlanActivatedMax' );
										} else {
								...
								$scope.bankWait = true;
								Member.buyPlan ( options ).$promise.then ( function ( res ) {
									if ( res.url ) {
