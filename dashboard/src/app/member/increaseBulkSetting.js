 ...
 */
app.controller ( 'increaseBulk', ['$scope', '$log', 'Session', 'appMessenger', '$uibModal', 'PREFIX', 'InternetPlan', 'englishNumberFilter', 'translateNumberFilter', '$window',
	function ( $scope, $log, Session, appMessenger, $uibModal, PREFIX, InternetPlan, englishNumberFilter, translateNumberFilter, $window ) {
			...
			InternetPlan.find (
				...
				$uibModal.open ( {
					...
					controller:    ['$scope', '$uibModalInstance', 'Member', function ( $scope, $uibModalInstance, Member ) {
						...
						var amount = 0;
						var price = 0;
						$scope.bulk.price = translateNumberFilter ( plan[0].extraBulkPrice * 10 );
						...
						$scope.$watch ( 'bulk.amount', function ( newVal, oldVal ) {
							if ( typeof newVal !== 'undefined' ) {
								...
								$scope.bulk.amount = translateNumberFilter ( $scope.bulk.amount );
						...
						$scope.payment = function () {
							...
							Member.buyBulk ( options ).$promise.then ( function ( res ) {
								if ( res.url ) {
