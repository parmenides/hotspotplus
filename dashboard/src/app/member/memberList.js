...
app.controller ( 'memberList', [ '$scope', '$state', '$log', 'translateFilter', 'Business', 'InternetPlan', 'Member', 'uiGridConstants', '$http', 'genericService', 'Session', '$uibModal', 'PREFIX', 'appMessenger', 'translateNumberFilter', 'englishNumberFilter', 'credit', 'FileSaver', 'Blob',
	function ( $scope, $state, $log, translateFilter, Business, InternetPlan, Member, uiGridConstants, $http, genericService, Session, $uibModal, PREFIX, appMessenger, translateNumberFilter, englishNumberFilter, credit, FileSaver, Blob ) {
			...
			columnDefs:               [
				{
					...
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.editMember(row)">{{row.entity.username}}</a>',
				...
				},
				{
					...
					enableColumnMenu: false,
					headerCellFilter: 'translate',
					cellTemplate:     '<div class="ui-grid-cell-contents"><span ng-if="row.entity.internetPlanName">{{row.entity.internetPlanName}}</span><span ng-if="!row.entity.internetPlanName">-</span></div>'
				},
				{
					...
					enableColumnMenu: false,
					headerCellFilter: 'translate',
					cellTemplate:     '<div class="ui-grid-cell-contents">{{row.entity.upload | number | translateNumber }}&nbsp;{{"dashboard.Mbps" | translate}}</div>'
				},
				{
					...
					enableColumnMenu: false,
					headerCellFilter: 'translate',
					cellTemplate:     '<div class="ui-grid-cell-contents">{{row.entity.download | number | translateNumber }}&nbsp;{{"dashboard.Mbps" | translate}}</div>'
				},
				{
					...
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.activateMember(row)" uib-popover={{grid.appScope.activateMemberHelpText}} popover-trigger="mouseenter"' +
				...
				},
				{
					...
					cellClass:        'center',
					headerCellClass:  'headerCenter',
					cellTemplate:     '<a class="btn btn-link" type="button" ng-click="grid.appScope.refreshPlan(row)" uib-popover={{grid.appScope.refreshPlanHelpText}} popover-trigger="mouseenter"' +
				...
				},
				{
					...
					cellClass:        'center',
					headerCellClass:  'headerCenter',
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.editPassword(row.entity)"><i class="fa fa-key"></i></a>'
				},
				{
					...
					cellClass:        'center',
					headerCellFilter: 'translate',
					cellTemplate:     '<div class="ui-grid-cell-contents" ng-click="grid.appScope.showPlanHistory(row)" uib-popover={{grid.appScope.showPlanHistoryHelpText}} popover-trigger="mouseenter"' +
				...
				},
				{
					...
					cellClass:        'center',
					headerCellClass:  'headerCenter',
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.editMember(row)"><i class="fa  fa-pencil"></i></a>'
				},
				{
					...
					cellClass:        'center',
					headerCellClass:  'headerCenter',
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.removeMember(row)"><i class="fa  fa-trash"></i></a>'
				}
			],
		...
		$scope.addMember = function () {
			...
			Business.internetPlans ( { id: businessId } ).$promise.then ( function ( internetPlans ) {
						...
						controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
							...
							$scope.save = function ( sendMessage ) {
								...
								if ( $scope.member.mobile ) {
									$scope.member.mobile = englishNumberFilter ( $scope.member.mobile )
								}
								var planId = $scope.member.selectedInternetPlan.id;
								delete $scope.member.selectedInternetPlan;
								Business.members.create ( { id: businessId }, $scope.member ).$promise.then ( function ( res ) {
									var memberId = res.id;
									InternetPlan.assignPlanToMember ( {
										...
										appMessenger.showSuccess ( 'member.createSuccessFull' );
										getPage ();
										$uibModalInstance.close ()
									}, function ( error ) {
										appMessenger.showError ( 'member.assignInternetPlanUnSuccessFull' )
									} )
								}, function ( err ) {
		...
		$scope.sendPassword = function ( memberId ) {
			Member.sendPassword ( { memberId: memberId, businessId: businessId } ).$promise.then ( function ( result ) {
				...
				if ( error && error.data.error && error.data.error.message ) {
					...
					return;
				} else if ( error == 'memberNotFound' || error == 'noMobileNumber' || error == 'balanceNotEnough' ) {
		...
		$scope.editMember = function ( row ) {
			...
			Member.loadMember ( { businessId: businessId, memberId: memberId } ).$promise.then ( function ( member ) {
				$uibModal.open ( {
					backdrop:      true,
					animation:     true,
					keyboard:      true,
					...
					controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
						...
						$scope.save = function () {
							...
							if ( !$scope.member.selectedInternetPlan ) {
								...
								var planId = $scope.member.selectedInternetPlan.id;
								delete $scope.member.selectedInternetPlan;
								Business.members.updateById ( {
									id: businessId,
									fk: memberId
								}, $scope.member ).$promise.then ( function ( res ) {
									if ( planId != res.internetPlanId ) {
										InternetPlan.assignPlanToMember ( {
											memberId: memberId,
											planId:   planId
										} ).$promise.then ( function ( res ) {
											appMessenger.showSuccess ( 'member.updateSuccessFull' );
											getPage ();
											$uibModalInstance.close ()
										}, function ( err ) {
											appMessenger.showError ( 'member.assignInternetPlanUnSuccessFull' )
							...
							function ( $scope, $uibModalInstance ) {
								...
								$scope.save = function () {
									...
									if ( $scope.memberByGroup.mobile ) {
										$scope.memberByGroup.mobile = englishNumberFilter ( $scope.memberByGroup.mobile );
									}
									if ( $scope.memberByGroup.mobile && $scope.memberByGroup.mobile.length < 10 ) {
										validationError = true;
										appMessenger.showError ( 'member.memberByGroupCheckMobile' );
									}
									if ( $scope.memberByGroup.internetPlan ) {
										$scope.memberByGroup.internetPlanId = $scope.memberByGroup.internetPlan.id;
									} else {
										validationError = true;
										appMessenger.showError ( 'member.memberByGroupRequiredInternetPlan' );
									}
									if ( !$scope.memberByGroup.mobile && !$scope.memberByGroup.reserveCode && !$scope.memberByGroup.tourCode && !$scope.memberByGroup.passportNumber && !$scope.memberByGroup.nationalCode ) {
										validationError = true;
										appMessenger.showError ( 'member.memberByGroupRequiredField' );
									}
									if ( !$scope.memberByGroup.count || ($scope.memberByGroup.count < 3 || $scope.memberByGroup.count > 30) ) {
										validationError = true;
										appMessenger.showError ( 'member.memberByGroupCheckCount' );
									}
									if ( !validationError ) {
										if ( $scope.memberByGroup.internetPlan && $scope.memberByGroup.internetPlanId ) {
											delete $scope.memberByGroup.internetPlan;
										}
										$scope.loading = true;
										Member.createMembersByGroup ( $scope.memberByGroup ).$promise.then ( function ( renderedHtmlAsString ) {
											var data = new Blob ( [ renderedHtmlAsString.html ], { type: 'text/html' } );
											FileSaver.saveAs ( data, 'users.html' );
											appMessenger.showSuccess ( 'member.memberByGroupCreateSuccessfully' );
											$uibModalInstance.close ();
											$scope.loading = false;
											getPage ();
										}, function ( error ) {
				...
				$uibModal.open ( {
					...
					controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
						...
						$scope.uploadCsv = function () {
							$uibModalInstance.close ()
							appMessenger.showSuccess ( 'member.membersImportedSuccessfully' );
							var internetPlanId = null;
							if ( $scope.selectedInternetPlan && $scope.selectedInternetPlan.id ) {
		...
		$scope.removeMember = function ( row ) {
			genericService.showConfirmDialog ( {
				...
				yesCallback: function () {
					...
					Business.members.destroyById ( { id: businessId }, { fk: memberId } ).$promise.then ( function ( business ) {
					...
					controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
						...
						$scope.yes = function () {
							...
							Business.members.updateById ( {
								...
								appMessenger.showSuccess ( 'member.changeStatusSuccessFull' )
								getPage ()
							}, function ( err ) {
								appMessenger.showError ( 'member.changeStatusUnSuccessFull' )
							} )
							$uibModalInstance.close ()
						}
					} ]
		...
		$scope.searchMember = function () {
			if ( $scope.memberUsername ) {
				...
				var usernamePattern = '/' + $scope.memberUsername + '.*@' + businessId + '$/i';
		...
		var getPage = function ( inputFilter ) {
			...
			Business.members.count ( { id: businessId, where: inputFilter } ).$promise.then ( function ( result ) {
				$scope.gridOptions.totalItems = result.count;
				$scope.paginationOptions.totalItems = result.count;
			}, function ( error ) {
				$log.error ( error )
			} );
			Business.members ( options ).$promise.then ( function ( members ) {
				Business.internetPlans ( { id: businessId } ).$promise.then ( function ( internetPlans ) {
					Member.loadMemberUsage ( { members: members } ).$promise.then ( function ( usage ) {
