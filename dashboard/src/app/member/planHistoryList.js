 ...
 */
app.controller ( 'planHistoryList', [ '$scope', '$log', 'translateFilter', '$uibModalInstance', 'InternetPlan', 'Member', 'uiGridConstants', 'Session', 'PREFIX', 'appMessenger',
	function ( $scope, $log, translateFilter, $uibModalInstance, InternetPlan, Member, uiGridConstants, Session, PREFIX, appMessenger ) {
		...
		$scope.gridOptions = {
			...
			columnDefs:               [
				...
				},
				{
					...
					enableColumnMenu: false,
					headerCellFilter: 'translate',
					cellTemplate:     '<div class="ui-grid-cell-contents"><span ng-if="row.entity.assignDate">' +
				...
				},
				{
					...
					enableColumnMenu: false,
					headerCellFilter: 'translate',
					cellTemplate:     '<div class="ui-grid-cell-contents"><span ng-if="row.entity.duration">{{row.entity.duration | translateNumber }} ' +
				...
				},
				{
					...
					enableColumnMenu: false,
					headerCellFilter: 'translate',
					cellTemplate:     '<div class="ui-grid-cell-contents"><span ng-if="row.entity.type">{{row.entity.type | translate}}</span><span ng-if="!row.entity.type">-</span></div>'
				},
				{
					...
					enableColumnMenu: false,
					headerCellFilter: 'translate',
					cellTemplate:     '<div class="ui-grid-cell-contents"><span ng-if="row.entity.accessType">{{row.entity.accessType | translate}}</span><span ng-if="!row.entity.accessType">-</span></div>'
				},
				{
		...
		var getPage = function () {
			...
			Member.loadMemberInternetPlans ( {
				...
				if ( res.internetPlanHistory && res.internetPlanHistory.length != 0 ) {
					...
					$scope.paginationOptions.totalItems = res.internetPlanHistory.length;
					$scope.gridOptions.data = res.internetPlanHistory;
				} else {
					$scope.gridOptions.totalItems = 1;
					$scope.paginationOptions.totalItems = 1;
					$scope.gridOptions.data.push ( { name: $scope.member.internetPlanName } );
				}
			}, function ( error ) {
				$log.error ( error );
