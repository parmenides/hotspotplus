...
app.controller ( 'nasList', [ '$scope', '$state', '$log', 'translateFilter', 'Business', 'Nas', 'uiGridConstants', '$http', 'genericService', '$uibModal', 'PREFIX', 'Session', 'appMessenger',
	function ( $scope, $state, $log, translateFilter, Business, Nas, uiGridConstants, $http, genericService, $uibModal, PREFIX, Session, appMessenger ) {
			...
			columnDefs:               [
				{
					...
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.editNas(row)">{{row.entity.title}}</a>',
				},
				{
				...
				},
				{
					...
					headerCellFilter: 'translate',
					cellTemplate:     '<a class="btn btn-warning btn-block" ng-click="grid.appScope.downloadScript(row)" ><i class="fa fa-fw fa-download"></i></a>'
				},
				{
					...
					headerCellClass:  'headerCenter',
					headerCellFilter: 'translate',
					cellTemplate:     "<a  class='btn btn-link btn-block' ng-click='grid.appScope.editNas(row)' ><i class='fa fa-fw fa-pencil'></i></a>"
				},
				{
					...
					headerCellClass:  'headerCenter',
					headerCellFilter: 'translate',
					cellTemplate:     '<a class="btn btn-link btn-block" ng-click="grid.appScope.removeNas(row)" ><i class="fa fa-fw fa-trash"></i></a>'
				}
			],
		...
		$scope.removeNas = function ( row ) {
			genericService.showConfirmDialog ( {
				...
				yesCallback: function () {
					...
					Business.nas.destroyById ( { id: businessId }, { fk: nasId } ).$promise.then ( function ( res ) {
				...
				controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
					...
					$scope.save = function () {
						if ( verifyNas ( $scope.nas ) ) {
							...
							if ( $scope.nas.accessPointType !== 'mikrotik' && (!$scope.nas.mac || !verifyMac ( $scope.nas.mac )) ) {
							...
							Business.nas.create ( { id: businessId }, $scope.nas ).$promise.then ( function ( res ) {
		...
		$scope.downloadScript = function ( row ) {
			...
			var nasId = row.entity.id
			if ( row.entity.accessPointType === "mikrotik" ) {
				Business.nas.findById ( { id: businessId, fk: nasId } ).$promise.then ( function ( nas ) {
					$scope.nas = nas
					window.location.href = Window.API_URL + '/api/radius/downloadScripts/{2}?businessId={1}&nasId={0}'.replace ( '{2}', nas.title ).replace ( '{1}', businessId ).replace ( '{0}', nasId );
				}, function ( error ) {
					$log.error ( error )
				} )
			} else {
		...
		$scope.downloadAccountingScript = function ( row ) {
			...
			Business.nas.findById ( { id: businessId, fk: nasId } ).$promise.then ( function ( nas ) {
				$scope.nas = nas
				window.location.href = '/api/radius/downloadAccountingScripts/{2}?businessId={1}&nasId={0}'.replace ( '{2}', nas.title ).replace ( '{1}', businessId ).replace ( '{0}', nasId );
			}, function ( error ) {
				$log.error ( error )
			} )
		}
					...
					controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
						...
						$scope.save = function () {
							if ( verifyNas ( $scope.nas ) ) {
								...
								Business.nas.updateById ( {
									id: businessId,
									fk: nasId
								}, $scope.nas ).$promise.then ( function ( res ) {
		...
		$scope.reloadIps = function () {
			Business.reloadIps ( { businessId: businessId } ).$promise.then ( function ( result ) {
				appMessenger.showSuccess ( 'nas.reloadIpRequestSuccessFull' );
			}, function ( error ) {
				appMessenger.showError ( 'nas.reloadIpRequestUnSuccessFull' );
				if ( error.data && error.data.error && error.data.error.message == 'nasList empty' ) {
		...
		var getPage = function () {
			...
			Business.nas.count ( { id: businessId } ).$promise.then ( function ( result ) {
				$scope.gridOptions.totalItems = result.count
				$scope.paginationOptions.totalItems = result.count
			}, function ( error ) {
				$log.error ( error )
			} );
			Business.nas ( options ).$promise.then ( function ( nases ) {
