...
app.controller('netflowReport', [
  ...
  function(
    ...
    $scope.gridOptions = {
      ...
      columnDefs: [
...

        {
          ...
          cellTemplate:
            '<div style="padding: 8px 15px;">{{row.entity.sourceIp }}:{{row.entity.sourcePort }}</div>',
        },
        {
          ...
          cellClass: 'text-right',
          cellTemplate:
            '<div style="padding: 8px 15px;">{{row.entity.destinationIp }}:{{row.entity.destinationPort }}</div>',
        },
      ],
    ...
    var getPage = function() {
      ...
      Business.getNetflowReport(options).$promise.then(function(netflowReport) {
