/**
 * Created by rezanazari on 5/1/17.
 */

app.controller('netflowReport', ['$scope', '$state', 'Business', '$log', 'translateFilter', 'uiGridConstants', '$http', 'genericService', 'Session', 'persianDateFilter', 'translateNumberFilter', 'englishNumberFilter',
    function ($scope, $state, Business, $log, translateFilter, uiGridConstants, $http, genericService, Session, persianDateFilter, translateNumberFilter, englishNumberFilter) {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
...

                {
                    ...
                    cellTemplate: '<div style="padding: 8px 15px;">{{row.entity.sourceIp }}:{{row.entity.sourcePort }}</div>'
                },
                {
                    displayName: 'report.dstAddress',
                    field: 'destinationIp',
                    enableColumnMenu: false,
                    enableHiding: false,
                    enableSorting: false,
                    ...
                    headerCellClass: 'headerCenter',
                    cellClass: 'text-right',
                    cellTemplate: '<div style="padding: 8px 15px;">{{row.entity.destinationIp }}:{{row.entity.destinationPort }}</div>'
                }
            ],
        ...
        var getPage = function () {
            ...
            if ($scope.destinationIp) {
                options.destinationIp = $scope.destinationIp;
            }
            if ($scope.sourceIp) {
                options.sourceIp = $scope.sourceIp;
            }
            ...
            Business.getNetflowReport(options)
                .$promise.then(function (netflowReport) {
                $scope.gridOptions.totalItems = netflowReport.count;
                $scope.paginationOptions.totalItems = netflowReport.count;
                $scope.gridOptions.data = netflowReport.data;
            }), function (error) {
                $log.error(error);
            };
        };
