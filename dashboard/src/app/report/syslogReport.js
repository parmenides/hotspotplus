 ...
 */
app.controller ( 'sysLogReport', ['$scope', '$state', 'Business', '$log', 'translateFilter', 'uiGridConstants', '$http', 'genericService', 'Session', 'persianDateFilter', 'translateNumberFilter', 'englishNumberFilter',
	function ( $scope, $state, Business, $log, translateFilter, uiGridConstants, $http, genericService, Session, persianDateFilter, translateNumberFilter, englishNumberFilter ) {
		...
		$scope.gridOptions = {
			...
			columnDefs:               [
				...
				},
				{
					...
					cellTemplate:     '<div style="padding: 8px 15px;">{{row.entity.url}}</div>'
				}
			],
		...
		var getPage = function () {
			...
			Business.getSyslogReport ( options )
				.$promise.then ( function ( syslogReport ) {
