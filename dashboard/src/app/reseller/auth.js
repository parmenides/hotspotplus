...
app.controller ( 'ResellerSignUpController', [ '$scope', '$log', 'Reseller', '$state', 'appMessenger', 'englishNumberFilter',
	function ( $scope, $log, Reseller, $state, appMessenger, englishNumberFilter ) {
		...
		$scope.signUp = function () {
			...
			Reseller.create ( $scope.reseller ).$promise.then ( function ( result ) {
...
app.controller ( 'ResellerSignInController', [ '$scope', '$log', 'Reseller', '$state', 'appMessenger', 'Session', 'LoopBackAuth', 'roleService', 'englishNumberFilter', 'errorMessenger',
	function ( $scope, $log, Reseller, $state, appMessenger, Session, LoopBackAuth, roleService, englishNumberFilter, errorMessenger ) {
		$scope.credential = {}
		$scope.authError = null
		$scope.signIn = function () {
			$scope.credential.username = englishNumberFilter($scope.credential.username)
			Reseller.login ( $scope.credential ).$promise.then ( function ( result ) {
				var SessionData = {};
				roleService.getRoles ( LoopBackAuth.currentUserId ).then ( function ( roles ) {
		...
		$scope.signOut = function () {
			Reseller.logout ().$promise.then ( function () {
				$state.go ( 'access.resellerSignIn' )
