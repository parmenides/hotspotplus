...
app.controller ( 'resellerBusinessList', [ '$scope', '$state', '$log', 'translateFilter', 'Business', 'uiGridConstants', 'genericService', 'persianDateFilter', 'translateNumberFilter', 'credit', 'appMessenger', '$uibModal', 'PREFIX', 'Reseller', 'Session', 'englishNumberFilter',
	function ( $scope, $state, $log, translateFilter, Business, uiGridConstants, genericService, persianDateFilter, translateNumberFilter, credit, appMessenger, $uibModal, PREFIX, Reseller, Session, englishNumberFilter ) {
			...
			columnDefs:               [
				{
					...
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.editBusiness(row)">{{row.entity.title}}</a>',
				...
				},
				{
					...
					enableColumnMenu: false,
					headerCellFilter: 'translate',
					cellTemplate:     '<div class="ui-grid-cell-contents"><span ng-if="row.entity.mobile">{{row.entity.mobile | translateNumber }}</span><span ng-if="!row.entity.mobile">-</span></div>'
				},
				{
					...
					enableColumnMenu: false,
					headerCellFilter: 'translate',
					cellTemplate:     '<div class="ui-grid-cell-contents">{{"business."+(row.entity.services.id || "freeService") | translate }}</div>'
				},
				{
					...
					enableColumnMenu: false,
					headerCellFilter: 'translate',
					cellTemplate:     '<div class="ui-grid-cell-contents">{{row.entity.durationInMonths | translateNumber }}&nbsp;{{"general.month" | translate }}</div>'
				},
				{
				...
				},
				{
					...
					enableColumnMenu: false,
					headerCellFilter: 'translate',
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.openLoginPage(row)" >{{"general.login"|translate}}</a>'
				},
				{
		...
		$scope.openLoginPage = function () {
			window.open ( 'http://' + window.location.host + '/' + window.location.pathname + '#/access/signin' );
		};

		$scope.addBusiness = function () {
			$scope.business = {};
			Business.loadServices ().$promise.then ( function ( services ) {
				var activeTariff = reseller.commissionRate || services.RESELLERS_COMMISSION_RATE;
				$uibModal.open ( {
					backdrop:      true,
					animation:     true,
					keyboard:      true,
					...
					controller:    [ '$scope', '$uibModalInstance', 'appMessenger', function ( $scope, $uibModalInstance, appMessenger ) {
						...
						for ( var i in services.packages ) {
							$scope.data.packages[ services.packages[ i ][ 'id' ] ] = services.packages[ i ];
						...
						$scope.updateModulePrice = function () {
							...
							for ( var k in $scope.data.modules ) {
								...
								if ( selected === true && $scope.data.packages[ $scope.data.packagesId ].modules ) {
									//$log.debug ( $scope.data.packages[ $scope.data.packagesId ] )
									$scope.data.modulePrice = $scope.data.modulePrice + $scope.data.packages[ $scope.data.packagesId ].modules[ k ].annuallyPrice;
								}
							}
						};
						...
						$scope.$watchGroup ( [ 'data.modules.log', 'data.modules.memberPanel', 'data.modules.payment' ], function () {
							$scope.updateModulePrice ();
						} )
						...
						$scope.save = function ( error ) {
							...
							if ( !error && !validationError ) {
								Reseller.createBusiness ( {
									...
									appMessenger.showSuccess ( 'profile.businessCreatedSuccessFully' );
									$uibModalInstance.close ();
									/*if ( res.url ) {
			...
			Reseller.findBusiness ( {
				...
				$uibModal.open ( {
					...
					controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
						...
						business.onlineUsers = translateNumberFilter ( business.onlineUsers );
						business.durationInMonths = translateNumberFilter ( business.durationInMonths );
						...
						$scope.business.subscriptionDate = new Date ( $scope.business.subscriptionDate );
						...
						$scope.save = function ( error ) {
							$scope.business.subscriptionDate = new Date ( $scope.business.subscriptionDate ).getTime ();
							$scope.business.onlineUsers = englishNumberFilter ( $scope.business.onlineUsers );
							$scope.business.durationInMonths = englishNumberFilter ( $scope.business.durationInMonths );
							var validationError = false;
							if ( !$scope.business.onlineUsers || isNaN ( $scope.business.onlineUsers ) ) {
								appMessenger.showError ( 'reseller.invalidOnlineUsers' );
								validationError = true;
							} else if ( $scope.business.onlineUsers > 5 && $scope.business.onlineUsers > ($scope.remainingOnlineUsers + $scope.oldOnlineUsers) ) {
								appMessenger.showError ( 'reseller.maxOnlineUsersRequired' );
								validationError = true;
							}
							if ( !$scope.business.durationInMonths || isNaN ( $scope.business.durationInMonths ) ) {
								appMessenger.showError ( 'reseller.invalidDurationInMonths' );
								validationError = true;
							} else if ( $scope.business.durationInMonths < 1 ) {
								appMessenger.showError ( 'reseller.minDurationInMonthsRequired' );
								validationError = true;
							} else if ( $scope.business.durationInMonths > 12 ) {
								appMessenger.showError ( 'reseller.maxDurationInMonthsRequired' );
								validationError = true;
							}
							if ( !error && !validationError ) {
								$scope.business.mobile = englishNumberFilter ( $scope.business.mobile );
								Reseller.updateBusiness ( { business: $scope.business } ).$promise.then ( function ( res ) {
		...
		var getPage = function () {
			...
			options.skip = ($scope.paginationOptions.pageNumber - 1) * $scope.paginationOptions.itemPerPage;
			options.limit = $scope.paginationOptions.itemPerPage;
			options.where = { resellerId: resellerId };

			Reseller.loadBusiness ( { options: options } ).$promise.then ( function ( data ) {
