...
app.controller ( 'resellerList', [ '$scope', '$log', 'translateFilter', 'Reseller', 'Invoice', 'uiGridConstants', 'genericService', 'persianDateFilter', 'translateNumberFilter', 'credit', 'appMessenger', '$uibModal', 'PREFIX', 'englishNumberFilter',
	function ( $scope, $log, translateFilter, Reseller, Invoice, uiGridConstants, genericService, persianDateFilter, translateNumberFilter, credit, appMessenger, $uibModal, PREFIX, englishNumberFilter ) {
			...
			columnDefs:               [
				{
					...
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.addOrEditReseller(row)">{{row.entity.title}}</a>'
				},
				{
				...
				},
				{
					...
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.activateReseller(row)"><i class="fa fa-circle-o" ng-if="!row.entity.active"></i>' +
				...
				},
				{
					...
					headerCellFilter: 'translate',
					cellClass:        'center',
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.deleteReseller(row)" ><i class="fa fa-fw fa-trash"></i></a>'
				}
			],
		...
		$scope.addOrEditReseller = function ( row ) {
			if ( row ) {
				...
				Reseller.findById ( { id: resellerId } ).$promise.then ( function ( res ) {
		...
		$scope.openResellerForm = function ( reseller, options ) {
			$uibModal.open ( {
				...
				controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
					...
					$scope.reseller.allowedOnlineUsers = translateNumberFilter ( $scope.reseller.allowedOnlineUsers );
					if ( $scope.reseller.durationInMonths ) {
						$scope.reseller.durationInMonths = translateNumberFilter ( $scope.reseller.durationInMonths );
					}
					if ( $scope.reseller.mobile ) {
						$scope.reseller.mobile = translateNumberFilter ( $scope.reseller.mobile );
					}
					...
					$scope.save = function ( error ) {
						$scope.reseller.subscriptionDate = new Date ( $scope.reseller.subscriptionDate ).getTime ();
						$scope.reseller.allowedOnlineUsers = englishNumberFilter ( $scope.reseller.allowedOnlineUsers );
						$scope.reseller.durationInMonths = englishNumberFilter ( $scope.reseller.durationInMonths );
						$scope.reseller.mobile = englishNumberFilter ( $scope.reseller.mobile );
						var validationError = false;
						if ( !$scope.reseller.allowedOnlineUsers || isNaN ( $scope.reseller.allowedOnlineUsers ) ) {
							appMessenger.showError ( 'general.invalidOnlineUsers' );
							validationError = true;
						}
						if ( !$scope.reseller.durationInMonths || isNaN ( $scope.reseller.durationInMonths ) || $scope.reseller.durationInMonths < 0 ) {
							appMessenger.showError ( 'general.invalidDurationInMonths' );
							validationError = true;
						}
						if ( !error && !validationError ) {
							if ( !$scope.reseller.id ) {
								Reseller.create ( $scope.reseller ).$promise.then ( function ( res ) {
								...
								Reseller.prototype$updateAttributes ( { id: $scope.reseller.id }, $scope.reseller ).$promise.then ( function ( res ) {
					...
					controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
						...
						$scope.yes = function () {
							...
							Reseller.prototype$updateAttributes ( {
								id: resellerId
							}, { active: active }, function ( res ) {
								appMessenger.showSuccess ( 'reseller.changeStatusSuccessFull' )
								getPage ()
							}, function ( err ) {
								appMessenger.showError ( 'reseller.changeStatusUnSuccessFull' )
							} );
							$uibModalInstance.close ()
						}
					} ]
		...
		$scope.deleteReseller = function ( row ) {
			genericService.showConfirmDialog ( {
				...
				yesCallback: function () {
					...
					Reseller.destroyById ( { id: resellerId } ).$promise.then ( function ( res ) {
		...
		$scope.showInvoices = function ( row ) {
			...
			Reseller.invoices ( options ).$promise.then ( function ( invoices ) {
				$uibModal.open ( {
					backdrop:      true,
					animation:     true,
					keyboard:      true,
		...
		var getPage = function () {
			...
			Reseller.count ().$promise.then ( function ( result ) {
				$scope.gridOptions.totalItems = result.count;
				$scope.paginationOptions.totalItems = result.count;
			}, function ( error ) {
				$log.error ( error )
			} );
			Reseller.find ( options ).$promise.then ( function ( resellers ) {
