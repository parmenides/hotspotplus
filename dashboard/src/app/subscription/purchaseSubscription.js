 ...
 */
app.controller ( 'purchaseSubscriptionController', [ '$scope', '$log', 'Session', 'Business', 'appMessenger', '$uibModal', 'PREFIX', 'Coupon', 'Invoice',
	function ( $scope, $log, Session, Business, appMessenger, $uibModal, PREFIX, Coupon, Invoice ) {
		...
		Business.findById ( { id: businessId } ).$promise.then ( function ( business ) {
			...
			Business.loadServices ().$promise.then ( function ( result ) {
		...
		$scope.buyPackage = function ( selectedPackage ) {
			$uibModal.open ( {
				...
				controller:    [ '$scope', '$uibModalInstance', '$log', 'Session', function ( $scope, $uibModalInstance, $log, Session, $state ) {
					$scope.close = function () {
						$uibModalInstance.close ();
					};
					$scope.premiumPackage = selectedPackage;
					$scope.premiumPackage.cost = $scope.premiumPackage.price - ($scope.premiumPackage.price * $scope.premiumPackage.discount);
					$scope.searchGiftCode = function ( giftCode ) {
						$scope.premiumPackage.annuallyPriceWithCode = $scope.premiumPackage.price - ($scope.premiumPackage.price * $scope.premiumPackage.discount);
						if ( giftCode ) {
							$scope.isSearchingCode = true;
							$scope.giftCodeValid = false;
							Coupon.verifyGiftCode ( {giftCode:giftCode} ).$promise.then ( function ( loadedCode ) {
								...
								if ( loadedCode ) {
									...
									if ( used < count ) {
										...
										var packagePrice = ($scope.premiumPackage.price - ($scope.premiumPackage.price * $scope.premiumPackage.discount));
										if ( unit === 'percent' ) {
											var discountAmount = $scope.premiumPackage.price * amount / 100;
											$scope.premiumPackage.priceWithCode = packagePrice - discountAmount;
										}
										if ( unit === 'toman' ) {
											$scope.premiumPackage.priceWithCode = packagePrice - amount;
										}
									} else {
				...
				Business.loadServices ().$promise.then ( function ( services ) {
					...
					$uibModal.open ( {
						...
						controller:    [ '$scope', '$uibModalInstance', '$log', 'Session', function ( $scope, $uibModalInstance, $log, Session, $state ) {
							...
							$scope.premiumPackage = selectedPackage;
							$scope.searchGiftCode = function ( giftCode ) {
								$scope.premiumPackage.annuallyPriceWithCode = $scope.premiumPackage.annuallyPrice;
								if ( giftCode ) {
									$scope.isSearchingCode = true;
									$scope.giftCodeValid = false;
									var options = { filter: {} };
									options.filter.where = { code: giftCode, ownerId: 'Admin' };
									Coupon.find ( options ).$promise.then ( function ( result ) {
										...
										if ( result[ 0 ] ) {
											...
											if ( used < count ) {
												...
												var amount = $scope.giftCodeFound.value.amount;
												if ( unit == 'percent' ) {
													var discountAmount = $scope.premiumPackage.annuallyPrice * amount / 100;
													$scope.premiumPackage.annuallyPriceWithCode = $scope.premiumPackage.annuallyPrice - discountAmount;
												}
												if ( unit == 'toman' ) {
													$scope.premiumPackage.annuallyPriceWithCode = $scope.premiumPackage.annuallyPrice - amount;
												}
											} else {
												$scope.giftCodeUsed = true;
											}
		...
		Business.findById ( { id: businessId } ).$promise.then ( function ( res ) {
			if ( res.services ) {
				...
				$scope.currentService.expired = currentTime.getTime () > $scope.currentService.expiresAt;
				$scope.currentService.isExpiring = currentTime.getTime () > ($scope.currentService.expiresAt - 432000000);
				$scope.currentService.remainingDays = ($scope.currentService.expiresAt - currentTime.getTime ()) / 86400000;
			} else {
			...
			if ( res.resellerId ) {
				...
				Business.getResellerMobile ( { businessId: businessId } ).$promise.then ( function ( result ) {
