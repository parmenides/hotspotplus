...
app.controller ( 'hotspotThemes', [ 'PREFIX', '$scope', '$log', '$state', 'Business', 'Theme', 'appMessenger', 'genericService', 'Session', '$uibModal', 'uploadModal',
	function ( PREFIX, $scope, $log, $state, Business, Theme, appMessenger, genericService, Session, $uibModal, uploadModal ) {
		...
		var getPage = function () {
			Theme.loadHotspotThemes ().$promise.then ( function ( themes ) {
				$scope.themes = themes;
		...
		$scope.editThemeSettings = function ( theme ) {
			$uibModal.open ( {
				...
				controller:    [ '$scope', '$uibModalInstance', function ( $scope, $uibModalInstance ) {
					...
					var formConfigEn = angular.copy ( $scope.themes[ themeId ].formConfigEn);
					$scope.themeConfig = angular.copy ( $scope.businessThemeConfig[ themeId ] );
					$scope.themeConfig.formConfig = [];
					$scope.themeConfig.formConfigEn = [];
					angular.forEach ( formConfig, function ( config, configIndex ) {
							angular.forEach ( $scope.businessThemeConfig[ themeId ].formConfig, function ( conf, index ) {
									$scope.themeConfig.formConfig[configIndex] = config
								if ( conf.label === config.label ) {
									$scope.themeConfig.formConfig[configIndex].required = conf.required;
									$scope.themeConfig.formConfig[configIndex].active = conf.active;
								}
							} );
						}
					);
					angular.forEach ( formConfigEn, function ( config, configIndex ) {
							angular.forEach ( $scope.businessThemeConfig[ themeId ].formConfigEn, function ( conf, index ) {
									$scope.themeConfig.formConfigEn[configIndex] = config
								if ( conf.label === config.label ) {
									$scope.themeConfig.formConfigEn[configIndex].required = conf.required;
									$scope.themeConfig.formConfigEn[configIndex].active = conf.active;
								}
							} );
						}
					);

					if ( getConfigIndexById ( $scope.themeConfig.formConfig, 'password' ) != -1 ) {
						$scope.confirmPasswordEnabled = $scope.themeConfig.formConfig[ getConfigIndexById ( $scope.themeConfig.formConfig, 'password' ) ].active;
					}
					if ( getConfigIndexById ( $scope.themeConfig.formConfigEn, 'password' ) != -1 ) {
						$scope.confirmPasswordEnabledEn = $scope.themeConfig.formConfigEn[ getConfigIndexById ( $scope.themeConfig.formConfigEn, 'password' ) ].active;
					}
					$scope.setPasswordState = function ( state, language ) {
						switch ( language ) {
							case 'fa':
								if ( state === false ) {
									$scope.themeConfig.formConfig[ getConfigIndexById ( $scope.themeConfig.formConfig, 'confirmPassword' ) ].active = state;
								}
								$scope.confirmPasswordEnabled = state;
								break;
							case 'en':
								if ( state === false ) {
									$scope.themeConfig.formConfigEn[ getConfigIndexById ( $scope.themeConfig.formConfigEn, 'confirmPassword' ) ].active = state;
								}
					...
					$scope.$watch ( 'themeConfig.verificationMethodFa', function ( newVal, oldVal ) {
						if ( newVal == 'mobile' ) {
							var mobileIndex = getConfigIndexById ( $scope.themeConfig.formConfig, 'mobile' );
					...
					$scope.$watch ( 'themeConfig.verificationMethodEn', function ( newVal, oldVal ) {
						if ( newVal == 'mobile' ) {
							var mobileIndexEn = getConfigIndexById ( $scope.themeConfig.formConfigEn, 'mobile' );
					...
					$scope.upload = function ( file, param ) {
						...
						uploadModal.showUploadModal ( {
								...
								saveCallback:        function ( result ) {
									if ( param === 'logo' ) {
										$scope.themeConfig[ 'logo' ] = $scope.themeConfig[ 'logo' ] || {};
										$scope.themeConfig[ 'logo' ].id = result.id;
										$scope.themeConfig[ 'logo' ].name = result.name;
									} else if ( param === 'background' ) {
										$scope.themeConfig[ 'background' ] = $scope.themeConfig[ 'background' ] || {};
					...
					$scope.save = function () {
						...
						Business.prototype$updateAttributes ( { id: businessId }, { themeConfig: $scope.businessThemeConfig } ).$promise.then ( function ( res ) {
					...
					$scope.deleteFile = function ( param ) {
						...
						if ( param === 'logo' && $scope.themeConfig[ 'logo' ].id ) {
							fileId = true;
						} else if ( param === 'background' && $scope.themeConfig[ 'background' ].id ) {
							fileId = true;
						}
						if ( fileId ) {
							$uibModal.open ( {
