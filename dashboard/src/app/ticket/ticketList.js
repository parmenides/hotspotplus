...
app.controller ( 'ticketListController', [ '$scope', '$state', '$log', 'translateFilter', 'Campaign', 'Ticket', 'uiGridConstants', '$http', 'genericService', 'Session', '$uibModal', 'PREFIX', 'appMessenger', 'Business',
	function ( $scope, $state, $log, translateFilter, Campaign, Ticket, uiGridConstants, $http, genericService, Session, $uibModal, PREFIX, appMessenger, Business ) {
			...
			columnDefs:               [
				{
					...
					cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.showTicketDetails(row.entity.id)">{{row.entity.subject}}</a>'
				},

				{
					...
					enableColumnMenu: false,
					headerCellFilter: 'translate',
					cellTemplate:     '<div class="ui-grid-cell-contents">{{"ticket." + row.entity.response + "Response" | translate }}</div>'
				},
				{
			...
			$uibModal.open ( {
				...
				controller:    [ '$scope', '$uibModalInstance', 'Business', function ( $scope, $uibModalInstance, Business ) {
					...
					$scope.save = function ( error ) {
						...
						Business.tickets.create ( { id: Session.business.id }, $scope.ticket ).$promise.then ( function ( res ) {
		...
		var getPage = function ( inputFilter ) {
			...
			if ( Session.userType === 'Business' ) {
				...
				Business.tickets.count ( { id: businessId, where: inputFilter } ).$promise.then ( function ( result ) {
				...
				Business.tickets ( options ).$promise.then ( function ( tickets ) {
				...
				Ticket.count ( { where: inputFilter } ).$promise.then ( function ( result ) {
					$scope.gridOptions.totalItems = result.count;
					$scope.paginationOptions.totalItems = result.count;
				}, function ( error ) {
					$log.error ( error )
				...
				options.filter.skip = ($scope.paginationOptions.pageNumber - 1) * $scope.paginationOptions.itemPerPage;
				options.filter.limit = $scope.paginationOptions.itemPerPage;
				Ticket.find ( options ).$promise.then ( function ( tickets ) {
					$scope.isSearching = false;
					$scope.gridOptions.data = tickets;
				}, function ( error ) {
					$log.error ( error )
