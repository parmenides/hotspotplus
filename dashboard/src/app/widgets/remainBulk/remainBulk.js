 ...
 */
app.directive ( 'remainBulk', ['PREFIX', '$log',
	function ( PREFIX, $log ) {
		return {
			scope: {
				params: '=options'
			...
			controller:  function ( $scope ) {
				...
				function checkBulk () {
					if ( $scope.params.remainBulk.value === '' || $scope.params.remainBulk.value === 'undefined' || $scope.params.bulk === 'undefined' ) {
						$scope.params.remainBulk.value = '';
						$scope.percent = 0;
					} else if ( $scope.params.remainBulk.value === -1 ) {
						$scope.params.remainBulk.value = 'unlimited';
						$scope.percent = 100;
					} else if ( $scope.params.remainBulk.value !== -1 && $scope.params.bulk !== 0 ) {
						if ( $scope.params.extraBulk == 0 ) {
							...
							switch ( $scope.params.remainBulk.type ) {
								case 'KB':
									...
									remainBulk = (($scope.params.remainBulk.value / 1024) / 1024).toFixed ( 0 );
									break;
								case 'MB':
									extraBulk = $scope.params.extraBulk * 1024;
									remainBulk = ($scope.params.remainBulk.value / 1024).toFixed ( 0 );
									break;
								case 'GB':
							...
							$scope.percent = Math.round ( ($scope.params.remainBulk.value / originalBulk) * 100 );
						} else {
							$scope.percent = Math.round ( ($scope.params.remainBulk.value / $scope.params.bulk) * 100 );
						}
					}
					$scope.chartOption = {
