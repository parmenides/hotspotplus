 ...
 */
app.directive ( 'sessionsReport', [ 'PREFIX', '$log', '$rootScope', 'ClientSession', 'uiGridConstants', 'translateFilter', 'persianDateFilter', 'translateNumberFilter', 'appMessenger', 'trimUsernameFilter',
	function ( PREFIX, $log, $rootScope, ClientSession, uiGridConstants, translateFilter, persianDateFilter, translateNumberFilter, appMessenger, trimUsernameFilter ) {
		return {
			scope:       {
				params: '=options'
			},
			controller:  function ( $scope ) {
				...
				function makeTable () {
					...
					$scope.gridOptions = {
						...
						columnDefs:               [
							...
							},
							{
								...
								cellClass:        'center',
								cellFilter:       'translateNumber',
								cellTemplate:     '<a class="btn btn-link" ng-click="grid.appScope.killOnlineSession(row)"><i class="fa fa-power-off"></i></a>'
					...
					$scope.killOnlineSession = function ( row ) {
						if ( row && row.entity ) {
							ClientSession.killOnlineSession ( { session: row.entity } )
								.$promise.then ( function ( result ) {
								/*$scope.gridOptions.data.splice ( index, 1 )*/
								appMessenger.showSuccess ( 'dashboard.killOnlineSessionSuccessFull' )
							}, function ( error ) {
								$log.error ( error )
								appMessenger.showError ( 'dashboard.killOnlineSessionUnSuccessFull' )
							} )
						}
					}
					...
					var getPage = function () {
						...
						ClientSession.getOnlineSessionCount ( { businessId: query.businessId } )
							.$promise.then ( function ( result ) {
							$scope.gridOptions.totalItems = result.count
							$scope.paginationOptions.totalItems = result.count

							ClientSession.getOnlineUsers ( query )
								.$promise.then ( function ( onlineUsers ) {
								switch ( onlineUsers.result.data ) {
									case 'noSession':
										$scope.result = translateFilter ( 'dashboard.noSessionOnlineUsers' )
										$scope.loading = false
										break
									default :
								...
								$log.error ( 'can not get online user info from data source: ' + error )
							}
						}, function ( error ) {
							$log.error ( error )
						} )
