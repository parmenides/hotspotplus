angular.module('masterHotspotApp').controller("baseCtrl", ['$scope', '$log', '$state', '$location', 'config',
...
angular.module('masterHotspotApp').controller("baseSignInCtrl", ['$scope', '$timeout', '$log', '$state', '$location', 'config', 'appMessenger', 'appService', 'errorMessage', 'englishNumberFilter', 'routerService', 'translateFilter', 'loadingModal', 'numberService', 'usernameService', '$sce',
    function ($scope, $timeout, $log, $state, $location, config, appMessenger, appService, errorMessage, englishNumberFilter, routerService, translateFilter, loadingModal, numberService, usernameService, $sce) {
        if (!config || !config.selectedThemeConfig) {
            $state.go('home');
            return;
        }
        ...
        $scope.signupDestination = "http://" + window.location.host + "/#!/home.html?&staticnasid=" + config.nasId + "&uamip=" + config.uamip + "&uamport=" + config.uamport;
            ...
            appService.signIn({
                ...
                function (error, result) {
                    ...
                    if (!result.active) {
                        ...
                        Raven.setUserContext({id: username, email: config.businessId + "@hotspotplus.ir"});
                        if (result.ok == true) {
                            loadingModal.show();
                            routerService.login(username, password, $form, function (error, result) {
                                loadingModal.hide();
                                if (error) {
                                    $log.error(error);
                                    if (error.message) {
...
angular.module('masterHotspotApp').controller("baseSignUpCtrl", ['$scope', '$log', '$state', '$location', 'config', 'appMessenger', 'appService', 'errorMessage', 'englishNumberFilter', 'routerService', 'translateFilter', 'loadingModal', 'numberService', 'usernameService', 'nationalCode', 'birthday',
    function ($scope, $log, $state, $location, config, appMessenger, appService, errorMessage, englishNumberFilter, routerService, translateFilter, loadingModal, numberService, usernameService, nationalCode, birthday) {
        if (!config || !config.selectedThemeConfig) {
            $state.go('home');
            return;
        }
        ...
        if (config.localLang === 'en') {
            ...
            $scope.verificationMethod = config.selectedThemeConfig.verificationMethodEn || 'mobile';
        } else {
            config.formConfig = config.selectedThemeConfig['formConfig'];
            $scope.signUpTitle = $scope.themeConfig.signUpTitle || 'signUpTitle';
            $scope.verificationMethod = config.selectedThemeConfig.verificationMethodFa || 'mobile';
        }
        ...
        $scope.$watchGroup(['user.birthDay', 'user.birthMonth', 'user.birthYear'], function (newValues, oldValues, scope) {
        ...
        $scope.$watchCollection('user', function (newVal, oldVal) {
            if (newVal.verificationMethod != oldVal.verificationMethod) {
                angular.forEach($scope.formConfig, function (conf) {
                    if (conf.label == 'mobile' && newVal.verificationMethod === 'mobile') {
                        conf.active = true;
                        conf.required = true;
                    } else if (conf.label == 'mobile' && newVal.verificationMethod != 'mobile') {
        ...
        $scope.signUp = function (error) {
            ...
            if ($scope.user.nationalCode && !nationalCode.isValid($scope.user.nationalCode)) {
            ...
            if ($scope.user.verificationMethod && $scope.user.verificationMethod === 'user') {
                $scope.user.verificationMethod = 'mobile';
            }
            ...
            if (!error && !validationError) {
                angular.forEach($scope.formConfig, function (formConfig) {
                    if (formConfig.active && formConfig.required) {
                        if ($scope.user[formConfig.label]) {
                            $scope.user[formConfig.label] = englishNumberFilter($scope.user[formConfig.label]);
                        }
                    }
                });
                if ($scope.user.birthday) {
                    ...
                    $scope.user.birthMonth = new Date($scope.user.birthday).getMonth() + 1;
                ...
                appService.createHotSpotMember($scope.user, function (error, result) {
                    ...
                    if (result.status === -1) {
                        ...
                        appMessenger.showInfo('memberExistPleaseContactOperatorForManualVerification');
                        $state.go("theme.signin");
...
angular.module('masterHotspotApp').controller("baseVerifyCtrl", ['$scope', '$log', '$state', '$location', 'config', 'appMessenger', 'appService', 'errorMessage', 'englishNumberFilter', 'routerService', 'translateFilter', 'loadingModal',
    function ($scope, $log, $state, $location, config, appMessenger, appService, errorMessage, englishNumberFilter, routerService, translateFilter, loadingModal) {
...
angular.module('masterHotspotApp').controller("baseStatusCtrl", ['$scope', '$log', '$state', '$location', 'config', 'appMessenger', 'appService', 'errorMessage', 'englishNumberFilter', 'routerService', 'translateFilter', 'loadingModal', '$stateParams', '$timeout',
    function ($scope, $log, $state, $location, config, appMessenger, appService, errorMessage, englishNumberFilter, routerService, translateFilter, loadingModal, $stateParams, $timeout) {
        if (!$stateParams) {
            $state.go("theme.signin");
        }
        if (!config.selectedThemeConfig) {
...
angular.module('masterHotspotApp').controller("baseInternetPlanListCtrl", ['$scope', '$log', '$state', '$location', 'config', 'appMessenger', 'appService', 'errorMessage', 'englishNumberFilter', 'routerService', 'translateFilter', 'loadingModal',
    function ($scope, $log, $state, $location, config, appMessenger, appService, errorMessage, englishNumberFilter, routerService, translateFilter, loadingModal) {
        if (!config.selectedThemeConfig) {
            $state.go('home');
            return;
        ...
        appService.findInternetPlan({businessId: businessId}, function (error, plans) {
            if (error) {
                errorMessage.show(error);
                return;
            }
        ...
        $scope.payment = function (selectedPlan) {
            ...
            if (!memberId || !businessId || !planId) {
                ...
                if (price == 0) {
                    ...
                    appService.assignFreePlanToMember(freePlan).then(function (result) {
                        loadingModal.hide();
                    ...
                    appService.payment(paymentData)
...
angular.module('masterHotspotApp').controller("baseForgetPasswordCtrl", ['$scope', '$log', '$state', '$location', 'config', 'appMessenger', 'appService', 'errorMessage', 'englishNumberFilter', 'routerService', 'translateFilter', 'loadingModal',
    function ($scope, $log, $state, $location, config, appMessenger, appService, errorMessage, englishNumberFilter, routerService, translateFilter, loadingModal) {
        if (!config.selectedThemeConfig) {
            $state.go('home');
            return;
...
angular.module('masterHotspotApp').controller("baseLangCtrl", ['$scope', '$log', '$state', 'config', '$translate', '$rootScope',
