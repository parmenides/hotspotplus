...
var app = angular.module ( 'masterHotspotApp', [
	'ngResource',
	'ngRoute',
	'ui.router',
	...
	'oc.lazyLoad',
	'ui.bootstrap'
] ).config ( [ '$stateProvider', '$urlRouterProvider', '$controllerProvider', '$compileProvider', '$filterProvider', '$provide', '$translateProvider', 'JQ_CONFIG', 'MODULE_CONFIG',
	function ( $stateProvider, $urlRouterProvider, $controllerProvider, $compileProvider, $filterProvider, $provide, $translateProvider, JQ_CONFIG, MODULE_CONFIG ) {

		$stateProvider
			.state ( 'home', {
				template:   '<div style="width: 80%;margin: 0 auto;text-align: center;margin-top: 250px;">Loading</div>',
				url:        '/home.html',
				controller: 'appCtrl'
			} )
			.state ( 'theme', {
				...
				resolve:     load ( [ 'hotspotplus.tpls.' + businessConfig.themeConfig.template,
					'themeStyles/' + businessConfig.selectedThemeId + '/' + businessConfig.themeConfig[ businessConfig.selectedThemeId ].style + '.css' ], JQ_CONFIG, MODULE_CONFIG )
			} )
			.state ( 'theme.signin', {
			...
			.state ( 'theme.forgetpassword', {
				templateUrl: businessConfig.themeConfig.template + 'ForgetPassword.tpl.html',
				controller:  businessConfig.themeConfig.controller + 'ForgetPasswordCtrl',
			...
			.state ( 'theme.selectVerification', {
				templateUrl: businessConfig.themeConfig.template + 'SelectVerification.tpl.html',
				controller:  businessConfig.themeConfig.controller + 'SelectVerificationCtrl',
				url:         '/selectVerification.html'
			} )
			.state ( 'theme.internetplans', {
				templateUrl: businessConfig.themeConfig.template + 'InternetPlan.tpl.html',
				controller:  businessConfig.themeConfig.controller + 'InternetPlanListCtrl',
			...
			.state ( 'theme.status', {
				...
				url:         '/status.html',
				params:      { online: null, ip: null, download: null, upload: null, uptime: null, sessionTime: null }
			} )
			.state ( 'theme.lang', {
		...
		$translateProvider.translations ( 'fa', {
			"RADIUS server is not responding":                       "آی پی روتر شما شناسایی نشد و یا secret روتر شما صحیح نیست با پشتیبانی تماس بگیرید.",
			"loyaltySignUpHeader":                                   "با عضویت در باشگاه مشتریان ما از اتصال به اینترنت پر سرعت بهره‌مند شوید.",
			...
			"password is a required argument":                       "پیام لوکال: نام کاربری و رمز عبور الزامی است.",
			"invalid username or password":                          "پیام لوکال: نام کاربری صحیح نیست.",
			"checkHotspotNetworkConnection":                         "روتر شبکه‌ی شما در دسترس نیست، از اتصال به شبکه‌ی هات‌اسپات اطمینان پیدا کنید.",
			"verifyHeaderText":                                      "کد تایید ۴ رقمی برای شما پیامک شد، کد را  پس از دریافت وارد کنید.",
			"alreadyMember":                                         "در حال حاضر شما عضو هستید و نمی توانید دوباره عضو شوید. اگر رمز عبور خود را فراموش کرده‌اید بر روی 'رمز را فراموش کرده‌ام' بزنید یا به متصدی شبکه مراجعه کنید.",
			"noPublicPlan":                                          "هیچ پلن اینترنتی وجود ندارد.",
			"thisDeviceIsLockedByOtherUser":                         "این دستگاه به نام کاربری دیگری اختصاص داده شده است، اگر رمز عبور خود را فراموش کرده‌اید از بازیابی رمز عبور استفاده کنید.",
			"timeout":                                               "به دلیل شلوغی شبکه‌ی وای‌فای و یا دور بودن از آنتن‌های وایرلس، اتصال شما به شبکه‌ی وای‌فای دچار اختلال شده است و درخواست‌های شما ارسال نمی‌شود.",
			"logout":                                                "خروج",
			"nasIdIsInvalid":                                        "شناسه‌ی روتر شما صحیح نیست. شناسه را از پروفایل روتر کپی و به عنوان nas identity تنظیم کنید.",
			...
			"memberExistPleaseContactOperatorForManualVerification": "برای فعال سازی این حساب کاربری به مسئول شبکه مراجعه کنید.",
			"pleaseContactOperatorForManualVerification":            "این کاربر ثبت شده است اما فعال نیست، برای فعال سازی این حساب کاربری به مسئول شبکه مراجعه کنید.",
			"freeInternetPlan":                                      "بسته ی رایگان",
			"freePlanActivationFailed":                              "خطایی در فعال سازی بسته ی اینترنت رایگان شما رخ داده است.",
			"freePlanActivated":                                     "بسته ی اینترنت رایگان شما فعال شد.",
			"signUpSuccess":                                         "عضویت شما با موفقیت انجام شد. ",
			"loading":                                               "چند لحظه صبر کنید",
			"mobile":                                                "موبایل",
			"waitOneMinute":                                         "شما باید حداقل {0} دقیقه برای دریافت پیامک کد تایید صبر کنید، سپس می توانید درخواست ارسال مجدد کد را صادر کنید",
			"resendVerification":                                    "ارسال مجدد کد",
			"requiredField":                                         "*",
			"userNotFound":                                          "کاربری با این مشخصات پیدا نشد",
			"generalError":                                          "خطایی در برنامه رخ داده است",
			"serverConnectionError":                                 "خطایی در ارسال اطلاعات شما رخ داده است، از اتصال به شبکه‌ی هات‌اسپات اطمینان پیدا کنید.",
			...
			"loginToByInternetPlan":                                 "برای خرید بسته ی اینترنت لازم است با نام کاربری و رمز عبور وارد شوید.",
			"usernameAndPasswordSent":                               "نام کاربری و رمز عبور برای شما پیامک شد",
			"forgotPasswordTitle":                                   "بازیابی رمز عبور",
			"forgotPasswordHeaderText":                              "نام کاربری و یا شماره موبایل خود را وارد کنید تا رمز برای شما پیامک شود.",
			...
			"606":                                                   "یک نفر دیگر با این نام کاربری در حال استفاده از اینترنت است، دقایقی دیگر تلاش کنید",
			...
			"noInternetPlanFound":                                   "بسته اینترنتی پیدا نشد. لطفا یک بسته اینترنت خریداری یا فعال کنید.",
			...
			"signUp":                                                "عضو می شوم",
			"signUpVerificationError":                               "تعداد درخواست‌های کد تایید شما از حد مجاز بیشتر شده است، به مدیر شبکه مراجعه کنید.",
			...
			"activateInternetPlan":                                  "برای فعال سازی بسته اینترنت خود با مسئول شبکه تماس بگیرید.",
			"activateUsername":                                      "این نام کاربری هنوز فعال نشده است. برای فعال شدن این نام کاربری می توانید تایید هویت پیامکی انجام دهید و یا به مسئول اینترنت مراجعه کنید.",
			...
			"defaultPlanActivatedMax":                               "شما فعلا مجاز به استفاده‌ی مجدد از این بسته نیستید.",
		...
		$translateProvider.translations ( 'en', {
			"checkHotspotNetworkConnection":                         "Your router is not accessible, Please make sure you are connected to the Hotspot Wifi",
			"loyaltySignUpHeader":                                   "Please join our customer club to connect the internet",
			...
			"alreadyMember":                                         "User exist, If you forgot your password please use 'password recovery' or contact network administrator",
			"verifyTitle":                                           "تایید موبایل",
			"enterVerificationCode":                                 "Enter verification code",
			"youAreConnected":                                       "شما با موفقیت به اینترنت متصل شدید.",
			"loggedInSuccessfully":                                  "شما با موفقیت وارد و به اینترنت متصل شدید.",
			"invalidCode":                                           "کد صحیح نیست",
			"pleaseContactOperatorForManualVerification":            "Sign up completed to activate your account, contact network administrator.",
			"memberExistPleaseContactOperatorForManualVerification": "User exist exist but is disabled, please contact network administrator.",
			"congrats":                                              "تبریک",
			"confirm":                                               "Confirm",
			"warning":                                               "هشدار",
			...
			"freePlanActivationFailed":                              "An Error Has Occurred in Activating Free Internet Package",
			"freePlanActivated":                                     "Free Plan Activated Successfully",
			"signUpSuccess":                                         "عضویت شما با موفقیت انجام شد.شما میتوانید با نام کاربری و رمز عبوری که برای شما پیامک شده لاگین کنید. ",
			"loading":                                               "Please Wait",
			"mobile":                                                "mobile",
			"waitOneMinute":                                         "شما باید حداقل {0} دقیقه برای دریافت پیامک کد تایید صبر کنید، سپس می توانید درخواست ارسال مجدد کد را صادر کنید",
			...
			"loginToByInternetPlan":                                 "For Buying Internet Package You Must Login First",
			"usernameAndPasswordSent":                               "نام کاربری و رمز عبور برای شما پیامک شد",
			...
			"606":                                                   "Another Person With This username is Using The Internet, Try Again Later",
			...
			"activateInternetPlan":                                  "For Activate Your Internet Package Call Receptionist",
			...
			"defaultPlanActivatedMax":                               "Default InternetPlan Activated Too Many Times",
	...
	.run ( [ 'config', '$log', '$location', 'JQ_CONFIG', 'MODULE_CONFIG', function ( config, $log, $location ) {

		angular.merge ( config, window.businessConfig, {
			config:                  null,
			INTERNET_PLANS_FIND_URL: "/InternetPlans?filter[where][businessId]={0}&filter[order]=price {1}",
	...
	.controller ( "appCtrl", [ '$scope', '$state', 'config', '$log', 'appService', 'routerService', '$location', 'appMessenger', 'translateFilter', 'errorMessage', '$rootScope',
		function ( $scope, $state, config, $log, appService, routerService, $rootScope ) {
			...
			if ( businessConfig && businessConfig.clearTextPassword && businessConfig.username ) {
				config.username = businessConfig.username;
				config.password = businessConfig.clearTextPassword;
			}
			config.selectedThemeConfig = businessConfig.themeConfig[ businessConfig.selectedThemeId ];
			if ( config.selectedThemeConfig ) {
				if ( config.selectedThemeConfig.logo && config.selectedThemeConfig.logo.id ) {
					config.logo = config.selectedThemeConfig.logo;
					config.logo.src = $scope.DownloadUrlPrefix + '/file/download/' + config.logo.id;
				} else {
					config.logo = {};
					config.logo.name = 'logo';
					config.logo.src = 'img/' + config.selectedThemeId + '/logo/logo.png';
				}
				if ( config.selectedThemeConfig.background && config.selectedThemeConfig.background.id ) {
					config.background = config.selectedThemeConfig.background;
					config.background.src = $scope.DownloadUrlPrefix + '/file/download/' + config.background.id;
				}
				else {
					config.background = {};
					config.background.src = 'img/' + config.selectedThemeId + '/background/' + config.selectedThemeConfig.style + '.jpg';
				}
				...
				document.body.style.backgroundImage = 'url(' + config.background.src + ')';
			}
