 ...
 */
angular.module ( 'masterHotspotApp' ).service ( 'appService', [ '$http', 'config', '$log', '$q', 'usernameService',
	function ( $http, config, $log, $q, usernameService ) {
		...
		this.recoverHotspotUser = function ( usernameOrMobile, clbk ) {
			...
			$http.post ( window.API_URL + '/Members/recoverHotspotUser', usernameOrMobile )
				.then ( function ( result ) {
					return clbk ( null, result.data );
				} ).catch ( function ( error ) {
				sendError ( "recoverHotspotUser", error, { usernameOrMobile: usernameOrMobile } );
				return clbk ( error );
			} );
		};

		this.loadRouterInfo = function ( nasInfo, clbk ) {
			$http.post ( window.API_URL + '/Nas/loadRouterInfo', nasInfo ).then ( function ( result ) {
				return clbk ( null, result.data );
			} ).catch ( function ( error ) {
		...
		this.loadConfig = function ( bizInfo, clbk ) {
			$http.post ( window.API_URL + '/Businesses/loadConfig', bizInfo ).then ( function ( result ) {
				return clbk ( null, result.data );
			} ).catch ( function ( error ) {
				var message = "bizIdIsInvalid";
				if ( error.status == 404 ) {
					message = "bizNotFound"
					sendError ( "loadConfig", "business profile not found", { bizInfo: bizInfo } );
				}
		...
		this.getMemberBalance = function ( memberInfo, clbk ) {
			$http.post ( window.API_URL + '/Members/getMemberBalance', memberInfo )
				.then ( function ( result ) {
					return clbk ( null, result.data );
				} ).catch ( function ( error ) {
		...
		this.signIn = function ( userInfo, clbk ) {
			userInfo.username = usernameService.concat ( userInfo.username, userInfo.businessId );
			$http.post ( window.API_URL + '/Members/signIn', userInfo )
				.then ( function ( result ) {
					return clbk ( null, result.data );
				} ).catch ( function ( error ) {
		...
		this.signUpCustomer = function ( userInfo, clbk ) {
			$http.post ( window.API_URL + '/Members/signUpCustomer', userInfo )
			 .then ( function ( result ) {
			 return clbk ( null, result.data );
			 } ).catch ( function ( error ) {
		...
		this.createHotSpotMember = function ( user, clbk ) {
			$http.post ( window.API_URL + '/Members/createHotSpotMember', user )
				.then ( function ( result ) {
					return clbk ( null, result.data );
				} ).catch ( function ( error ) {
		...
		this.findInternetPlan = function ( businessId, clbk ) {
			$http.post ( window.API_URL + '/InternetPlans/getPublicInternetPlans', businessId )
				.then ( function ( result ) {
					return clbk ( null, result.data );
				} ).catch ( function ( error ) {
		...
		this.verifyHotSpot = function ( user, clbk ) {
			$http.post ( window.API_URL + '/Members/verifyHotSpot', user )
				.then ( function ( result ) {
					return clbk ( null, result.data );
				} ).catch ( function ( error ) {
		...
		this.assignFreePlanToMember = function ( freePlan ) {
			var defer = $q.defer ();
			$http.post ( window.API_URL + '/InternetPlans/assignFreePlanToMember', freePlan )
				.then ( function ( result ) {
		...
		this.payment = function ( paymentData ) {
			...
			$http.post ( window.API_URL + '/Members/paySubscription', paymentData ).then ( function ( result ) {
		...
		this.checkDefaultInternetPlan = function ( defaultPlan, cb ) {
			$http.post ( window.API_URL + '/Members/checkDefaultPlan', defaultPlan ).then ( function ( result ) {
				return cb ( null, result.data );
			} ).catch ( function ( error ) {
				sendError ( "checkDefaultInternetPlan", error, { defaultPlan: defaultPlan } );
				return cb ( error );
			} );
		}

		this.createForeignHotSpotMember = function ( user, clbk ) {
			$http.post ( window.API_URL + '/Members/createForeignHotSpotMember', user )
				.then ( function ( result ) {
					return clbk ( null, result.data );
				} ).catch ( function ( error ) {
				sendError ( error, { "method": "createForeignHotSpotMember" }, { user: user } );
				return clbk ( error );
			} );
		};
	} ] )
	.service ( 'errorMessage', [ '$log', 'appMessenger', function ( $log, appMessenger ) {
