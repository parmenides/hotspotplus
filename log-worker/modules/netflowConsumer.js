...
const consumerGroup = new kafka.ConsumerGroup(consumerGroupOptions, [process.env.RAW_NETFLOW_TOPIC]);
...
consumerGroup.on('message', function (message) {
    try {
        ...
        getIpSession(hostIp).then(function (nasSession) {
            ...
            getMemberSession(bizId, nasId, localMemberIp).then(function (memberSession) {
                const netFlowLog = {
                    ...
                    businessId: bizId,
                    nasId: nasId,
                    creationDate: new Date(netFlowPkg.netflow['first_switched']).getTime(),
                    creationDateStr: new Date(netFlowPkg.netflow['first_switched']).toLocaleString("fa-IR", {timeZone: "Asia/Tehran"}),
                    flowDate: new Date(netFlowPkg.netflow['first_switched']).getTime(),
                    flowDateStr: new Date(netFlowPkg.netflow['first_switched']).toLocaleString("fa-IR", {timeZone: "Asia/Tehran"}),
                };
                kafkaProducer.send([{
