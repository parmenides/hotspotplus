...
export const processLogRequest = async () => {
    ...
    channel.consume(
        ...
        async (message) => {
            ...
            //todo: test this
            if (generalReportRequestTask.nas && generalReportRequestTask.nas.length > 0) {
                generalReportRequestTask.nasId = [];
                generalReportRequestTask.nas.map((nas) => {
                    generalReportRequestTask.nasId.push(nas.id);
                })
            }
