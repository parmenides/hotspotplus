...
const netflowGroupByIp = async (
    ...
    for (const indexName of indexNames) {
        try {
            const result = await aggregateNetflowByIp(
                indexName,
                fromDate,
                toDate);
            data = data.concat(result);
...
const aggregateNetflowByIp = async (netflowIndex: string,
                                    fromDate: Moment,
                                    toDate: Moment) => {
...
const createNetflowGroupByAggregate = (
    fromDate: Moment,
    toDate: Moment,
) => {
    return {
...
interface NetflowAggregateByIp {
    group_by_nas_ip: {
